          ...
          <div>
            <DiagramTooltip content="Debezium активно читает события из binlog. Offset сохранен: mysql-bin.000010:154. Все работает нормально.">
              <FlowNode variant="connector" tabIndex={0} size="sm">
                <div class="text-xs">
                  <span class="text-[var(--ink-muted)]">Day 1:</span>{' '}
                  <span class="text-emerald-400">Debezium reading</span>
                  <div class="font-mono text-[var(--ink-muted)] mt-1">mysql-bin.000010:154</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div>
            <DiagramTooltip content="Debezium offline из-за обновления, проблемы Kafka, или сетевого сбоя. Offset не обновляется.">
              <FlowNode variant="sink" tabIndex={0} size="sm">
                <div class="text-xs">
                  <span class="text-[var(--ink-muted)]">Day 3:</span>{' '}
                  <span class="text-amber-400">Debezium offline</span>
                  <div class="text-[var(--ink-subtle)] mt-1">(обновление, проблема Kafka...)</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div>
            <DiagramTooltip content="MySQL purge: binlog_expire_logs_seconds=7 дней истек. Файлы mysql-bin.000001 - 000012 удалены. Offset Debezium указывает на несуществующий файл.">
              <FlowNode variant="database" tabIndex={0} size="sm">
                <div class="text-xs">
                  <span class="text-[var(--ink-muted)]">Day 8:</span>{' '}
                  <span class="text-rose-400">MySQL purge</span>
                  <div class="text-[var(--ink-subtle)] mt-1">mysql-bin.000001-000012 deleted</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div>
            <DiagramTooltip content="Debezium возвращается online, пытается читать mysql-bin.000010. Файл не найден! Требуется full resnapshot базы данных.">
              <FlowNode variant="app" tabIndex={0} size="sm">
                <div class="text-xs">
                  <span class="text-[var(--ink-muted)]">Day 9:</span>{' '}
                  <span class="text-rose-400 font-semibold">ERROR!</span>
                  <div class="font-mono text-rose-400 mt-1">File not found!</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
      ...
      <DiagramContainer title="Последствия" color="neutral">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <DiagramTooltip content="Cannot replicate because the master purged required binary logs. Требуется resnapshot.">
            <div class="bg-rose-900/20 border border-rose-500/30 p-3 rounded-lg">
              <div class="text-rose-400 font-semibold text-sm mb-1">Full Resnapshot</div>
              <div class="text-xs text-[var(--ink-muted)]">Часы/дни для больших баз</div>
            </div>
          </DiagramTooltip>

          <DiagramTooltip content="CDC pipeline остановлен пока идет resnapshot. Новые события буферизуются или теряются.">
            <div class="bg-amber-900/20 border border-amber-500/30 p-3 rounded-lg">
              <div class="text-amber-400 font-semibold text-sm mb-1">Downtime</div>
              <div class="text-xs text-[var(--ink-muted)]">Потенциальная потеря событий</div>
            </div>
          </DiagramTooltip>
        </div>
      </DiagramContainer>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row gap-4 justify-center">
        <DiagramTooltip content="Старые binlog файлы (до текущего offset) могут быть безопасно удалены MySQL при binlog_expire_logs_seconds.">
          <div class="bg-emerald-900/20 border border-emerald-500/30 p-4 rounded-lg text-center">
            <div class="text-emerald-400 font-semibold">Старые binlog can be purged</div>
            <div class="text-xs text-[var(--ink-muted)] mt-1">Offset всегда актуален</div>
          </div>
        </DiagramTooltip>
        ...
        <DiagramTooltip content="Даже если business таблицы idle, heartbeat поддерживает offset. Нет риска position loss.">
          <div class="bg-blue-900/20 border border-blue-500/30 p-4 rounded-lg text-center">
            <div class="text-blue-400 font-semibold">Idle tables protected</div>
            <div class="text-xs text-[var(--ink-muted)] mt-1">Heartbeat каждые 10 сек</div>
          </div>
        </DiagramTooltip>
      </div>
    </div>
  ...
  return (
    <div class="space-y-6">
      ...
      <DiagramContainer title="Формула расчета Retention" color="blue">
        <div class="flex flex-col items-center gap-4">
          <div class="bg-[var(--bg-sunken)] p-4 rounded-lg font-mono text-center">
            <DiagramTooltip content="Базовая формула: определите максимальное время простоя Debezium и добавьте safety margin для защиты от непредвиденных ситуаций.">
              <span class="cursor-help">
                <span class="text-emerald-400">Retention</span>{' '}
                <span class="text-[var(--ink-muted)]">=</span>{' '}
                <span class="text-blue-400">Max Downtime</span>{' '}
                <span class="text-[var(--ink-muted)]">+</span>{' '}
                <span class="text-amber-400">Safety Margin</span>
              </span>
            </DiagramTooltip>
          </div>
        </div>
      ...
      <DiagramContainer title="Примеры конфигураций" color="neutral">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <DiagramTooltip content="Stable production: редкие сбои, max downtime 2 дня. Safety margin 2x дает 4 дня retention = 345600 секунд.">
            <FlowNode variant="connector" tabIndex={0}>
              <div class="text-xs text-left">
                <div class="text-emerald-400 font-semibold">Stable Production</div>
                <div class="text-[var(--ink-muted)] mt-1">Max downtime: 2 дня</div>
                <div class="text-[var(--ink-muted)]">Safety: 2x</div>
                <div class="font-mono text-blue-400 mt-2">345600s (4 дня)</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Frequent deployments: частые обновления, max downtime 1 день. Safety margin 3x дает 3 дня = 259200 секунд.">
            <FlowNode variant="connector" tabindex={0}>
              <div class="text-xs text-left">
                <div class="text-blue-400 font-semibold">Frequent Deployments</div>
                <div class="text-[var(--ink-muted)] mt-1">Max downtime: 1 день</div>
                <div class="text-[var(--ink-muted)]">Safety: 3x</div>
                <div class="font-mono text-blue-400 mt-2">259200s (3 дня)</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Dev/Test environment: может быть неделя простоя. Минимальный safety margin 1x = 7 дней = 604800 секунд.">
            <FlowNode variant="connector" tabindex={0}>
              <div class="text-xs text-left">
                <div class="text-amber-400 font-semibold">Dev/Test</div>
                <div class="text-[var(--ink-muted)] mt-1">Max downtime: 1 неделя</div>
                <div class="text-[var(--ink-muted)]">Safety: 1x</div>
                <div class="font-mono text-blue-400 mt-2">604800s (7 дней)</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Critical production: zero tolerance для потери данных. Max downtime 3 дня, safety 5x = 15 дней = 1296000 секунд.">
            <FlowNode variant="connector" tabindex={0}>
              <div class="text-xs text-left">
                <div class="text-rose-400 font-semibold">Critical Production</div>
                <div class="text-[var(--ink-muted)] mt-1">Max downtime: 3 дня</div>
                <div class="text-[var(--ink-muted)]">Safety: 5x</div>
                <div class="font-mono text-blue-400 mt-2">1296000s (15 дней)</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
        </div>
          ...
          <div class="text-center">
            <DiagramTooltip content="Меньше риск position loss, но больше disk space. Binlog файлы накапливаются.">
              <div class="cursor-help">
                <div class="text-lg text-emerald-400">+ Безопасность</div>
                <div class="text-sm text-[var(--ink-muted)] mt-2">- Disk space</div>
              </div>
            </DiagramTooltip>
          </div>
        </DiagramContainer>
        ...
        <DiagramContainer title="Меньше retention" color="rose" className="flex-1">
          <div class="text-center">
            <DiagramTooltip content="Экономия disk space, но выше риск position loss при длительном downtime.">
              <div class="cursor-help">
                <div class="text-lg text-emerald-400">+ Disk space</div>
                <div class="text-sm text-[var(--ink-muted)] mt-2">- Риск position loss</div>
              </div>
            </DiagramTooltip>
          </div>
        </DiagramContainer>
