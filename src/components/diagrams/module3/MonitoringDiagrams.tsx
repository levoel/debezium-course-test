          ...
          <div class="flex flex-col md:flex-row items-center justify-center gap-4">
            <DiagramTooltip content="MySQL binlog event содержит timestamp создания события (source.ts_ms). Debezium извлекает этот timestamp для расчета lag.">
              <FlowNode variant="database" tabIndex={0}>
                Binlog Event
                <br />
                <span class="text-xs text-[var(--ink-muted)]">source.ts_ms</span>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="Debezium connector сравнивает event timestamp с текущим временем: lag = NOW() - source.ts_ms. Результат expose через JMX.">
              <FlowNode variant="connector" tabIndex={0}>
                Debezium Connector
                <br />
                <span class="text-xs text-[var(--ink-muted)]">calculate lag</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div class="flex flex-col md:flex-row items-start justify-center gap-4">
            <DiagramTooltip content="MilliSecondsBehindSource — RECOMMENDED primary lag metric. Показывает задержку в миллисекундах. Более точный, чем SecondsBehindMaster.">
              <FlowNode variant="app" tabIndex={0} className="border-2 border-emerald-400">
                ...
                <br />
                <span class="text-xs text-emerald-400">Primary metric</span>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="SecondsBehindMaster — MySQL-calculated metric. Может быть -1 во время failover (это нормально). Используйте для cross-check с MilliSecondsBehindSource.">
              <FlowNode variant="app" tabIndex={0}>
                SecondsBehindMaster
                <br />
                <span class="text-xs text-[var(--ink-muted)]">Cross-check</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div class="flex flex-col md:flex-row items-center justify-center gap-4">
            <DiagramTooltip content="JMX Exporter (port 9404) expose metrics в Prometheus format. Prometheus scrapes endpoint каждые 15-30 секунд.">
              <FlowNode variant="connector" tabIndex={0}>
                JMX Exporter
                <br />
                <span class="text-xs text-[var(--ink-muted)]">:9404/metrics</span>
              </FlowNode>
            </DiagramTooltip>
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <DiagramContainer title="Ключевые метрики" color="emerald" className="flex-1">
          <div class="text-xs text-[var(--ink-default)] space-y-2">
            <div>
              <span class="text-emerald-400 font-mono">MilliSecondsBehindSource</span>
              <br />
              <span class="text-[var(--ink-muted)]">Задержка connector от source DB (ms)</span>
            </div>
            <div>
              <span class="text-emerald-400 font-mono">BinlogPosition</span>
              <br />
              <span class="text-[var(--ink-muted)]">Текущая позиция в binlog файле</span>
            </div>
            <div>
              <span class="text-emerald-400 font-mono">Connected</span>
              <br />
              <span class="text-[var(--ink-muted)]">1 = connected, 0 = disconnected</span>
            </div>
          </div>
        </DiagramContainer>
        ...
        <DiagramContainer title="Queue метрики (backpressure)" color="amber" className="flex-1">
          <div class="text-xs text-[var(--ink-default)] space-y-2">
            <div>
              <span class="text-amber-400 font-mono">QueueRemainingCapacity</span>
              <br />
              <span class="text-[var(--ink-muted)]">Свободное место в internal queue</span>
            </div>
            <div>
              <span class="text-amber-400 font-mono">QueueTotalCapacity</span>
              <br />
              <span class="text-[var(--ink-muted)]">Общий размер queue (default: 8192)</span>
            </div>
            <div class="text-[var(--ink-muted)] text-xs mt-2">
      ...
      <DiagramContainer title="SecondsBehindMaster = -1 во время failover" color="neutral">
        <div class="flex flex-col md:flex-row items-center justify-center gap-4 text-xs text-[var(--ink-default)]">
          <div class="flex items-center gap-2">
            <span class="bg-amber-500/20 px-2 py-1 rounded">Failover начат</span>
          </div>
          <Arrow direction="right" />
          <div class="flex items-center gap-2">
            <span class="bg-rose-500/20 px-2 py-1 rounded">SecondsBehindMaster = -1</span>
          </div>
          <Arrow direction="right" />
          <div class="flex items-center gap-2">
            <span class="bg-emerald-500/20 px-2 py-1 rounded">Reconnect (30-60s)</span>
          </div>
          <Arrow direction="right" />
          <div class="flex items-center gap-2">
            <span class="bg-emerald-500/20 px-2 py-1 rounded">Normal value</span>
          </div>
        </div>
        <div class="text-xs text-[var(--ink-muted)] text-center mt-3">
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="flex flex-col lg:flex-row gap-4">
        ...
        <DiagramContainer title="Tier 1: JMX Metrics" color="amber" className="flex-1">
          <div class="flex flex-col items-center gap-3">
            ...
            <div class="text-xs text-[var(--ink-muted)] space-y-1 text-center">
              <div class="font-mono">MilliSecondsBehindSource</div>
              <div class="font-mono">Connected</div>
              <div class="font-mono">IsGtidModeEnabled</div>
              <div class="font-mono">QueueRemainingCapacity</div>
            </div>
          </div>
        </DiagramContainer>
        ...
        <DiagramContainer title="Tier 2: CloudWatch" color="blue" className="flex-1">
          <div class="flex flex-col items-center gap-3">
            ...
            <div class="text-xs text-[var(--ink-muted)] space-y-1 text-center">
              <div class="font-mono">AuroraBinlogReplicaLag</div>
              <div class="font-mono">ChangeLogBytesUsed</div>
              <div class="font-mono">ChangeLogReadIOPs</div>
              <div class="font-mono">ChangeLogWriteIOPs</div>
            </div>
          </div>
        </DiagramContainer>
        ...
        <DiagramContainer title="Tier 3: Operational" color="emerald" className="flex-1">
          <div class="flex flex-col items-center gap-3">
            ...
            <div class="text-xs text-[var(--ink-muted)] space-y-1 text-center">
              <div class="font-mono">Heartbeat Events</div>
              <div class="font-mono">Signal Table Events</div>
              <div class="font-mono">Kafka Consumer Lag</div>
            </div>
          </div>
        </DiagramContainer>
      </div>
        ...
        <div class="flex flex-col gap-6">
          ...
          <div class="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
            ...
            <DiagramTooltip content="Debezium connector читает binlog и expose JMX metrics. Для Aurora критично мониторить и connector, и infrastructure.">
              <FlowNode variant="connector" tabIndex={0}>
                Debezium
                <br />
                <span class="text-xs text-[var(--ink-muted)]">JMX :9404</span>
              </FlowNode>
            </DiagramTooltip>
          ...
          <div class="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
            <div class="flex flex-col items-center gap-2">
              ...
              <div class="text-xs text-[var(--ink-muted)]">Aurora metrics</div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              ...
              <div class="text-xs text-[var(--ink-muted)]">Aggregation</div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              ...
              <div class="text-xs text-[var(--ink-muted)]">Visualization</div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              ...
              <div class="text-xs text-[var(--ink-muted)]">Alerting</div>
            </div>
          </div>
        </div>
      </DiagramContainer>
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <DiagramContainer title="Warning Alerts" color="amber" className="flex-1">
          <div class="text-xs text-[var(--ink-default)] space-y-2">
            <div class="flex justify-between">
              ...
              <span class="font-mono text-amber-400">&gt; 60s for 5m</span>
            </div>
            <div class="flex justify-between">
              <span>Queue utilization</span>
              <span class="font-mono text-amber-400">&gt; 80%</span>
            </div>
            <div class="flex justify-between">
              <span>No events</span>
              <span class="font-mono text-amber-400">&gt; 60s</span>
            </div>
          </div>
        </DiagramContainer>
        ...
        <DiagramContainer title="Critical Alerts" color="rose" className="flex-1">
          <div class="text-xs text-[var(--ink-default)] space-y-2">
            <div class="flex justify-between">
              <span>Binlog lag</span>
              <span class="font-mono text-rose-400">&gt; 300s for 5m</span>
            </div>
            <div class="flex justify-between">
              <span>Disconnected</span>
              <span class="font-mono text-rose-400">&gt; 2m</span>
            </div>
            <div class="flex justify-between">
              <span>GTID mode disabled</span>
              <span class="font-mono text-rose-400">immediate</span>
            </div>
          </div>
        </DiagramContainer>
      </div>
      ...
      <DiagramContainer title="Почему три уровня мониторинга?" color="neutral">
        <div class="text-xs text-[var(--ink-default)] space-y-3">
          <div>
            <span class="text-amber-400 font-bold">Сценарий:</span> JMX показывает lag = 100ms (отлично), но CloudWatch показывает AuroraBinlogReplicaLag = 300s (5 минут).
          </div>
          <div>
            <span class="text-blue-400 font-bold">Проблема:</span> Connector работает корректно, но Aurora cross-region replica отстает на 5 минут.
          </div>
          <div>
            <span class="text-rose-400 font-bold">Риск:</span> При regional failover последние 5 минут данных потеряны — replica не успела реплицировать.
          </div>
          <div>
            <span class="text-emerald-400 font-bold">Вывод:</span> Single-layer мониторинг (только JMX) пропустил бы эту проблему. Three-tier обеспечивает полную visibility.
          </div>
        </div>
      </DiagramContainer>
    </div>
