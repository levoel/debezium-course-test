      ...
      <DiagramContainer title="Incremental Snapshot: Non-Blocking Flow" color="emerald">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <DiagramTooltip content="Step 1: INSERT в signal table с type='execute-snapshot' запускает incremental snapshot. Можно указать конкретные таблицы.">
              <FlowNode variant="database" tabIndex={0}>
                Signal Received
                <br />
                <span className="text-xs text-[var(--ink-muted)]">execute-snapshot</span>
              </FlowNode>
            </DiagramTooltip>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <DiagramTooltip content="Step 2: Определяем границы chunk по primary key. Например, id >= 1 AND id < 2049 для chunk size 2048.">
              <FlowNode variant="app" tabIndex={0}>
                Select Chunk Range
                <br />
                <span className="text-xs text-[var(--ink-muted)]">WHERE id &gt;= N AND id &lt; N+2048</span>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="SELECT с границами chunk. Не блокирует таблицу — использует обычный SELECT с WHERE условием.">
              <FlowNode variant="database" tabIndex={0}>
                Read Chunk
                <br />
                <span className="text-xs text-[var(--ink-muted)]">2048 rows</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            ...
            <div className="text-xs text-[var(--ink-muted)]">+</div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <DiagramTooltip content="Step 4: После завершения chunk, Debezium переходит к следующему. При crash — resume с последнего chunk.">
              <FlowNode variant="connector" tabIndex={0}>
                Next Chunk
                <br />
                <span className="text-xs text-[var(--ink-muted)]">id &gt;= 2049</span>
              </FlowNode>
            </DiagramTooltip>
      ...
      <DiagramContainer title="Ключевое преимущество: Параллельная обработка" color="emerald" recommended>
        <div className="text-sm text-[var(--ink-default)] space-y-2">
          ...
          <div className="text-xs text-[var(--ink-muted)]">
            ...
            <br />
            <code className="bg-[var(--bg-sunken)] px-1 rounded">"incremental"</code> для snapshot, <code className="bg-[var(--bg-sunken)] px-1 rounded">"false"</code> для streaming.
          </div>
        </div>
      </DiagramContainer>
      ...
      <DiagramContainer title="Chunk Processing с Watermarks" color="amber">
        <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex flex-col items-center gap-2">
              <DiagramTooltip content="LOW watermark: Начало chunk. Например, id=1 для первого chunk с chunk size 2048.">
                <FlowNode variant="app" tabIndex={0} size="sm" className="border-2 border-blue-400">
                  LOW Watermark
                  <br />
                  <span className="text-xs text-[var(--ink-muted)]">id=1</span>
                </FlowNode>
              </DiagramTooltip>
            </div>
          ...
          <div className="w-full">
            <div className="text-xs text-[var(--ink-muted)] mb-2 text-center">Interleaving с Binlog Events</div>
            <div className="flex flex-col gap-2">
              {/* Chunk 1 */}
              <div className="flex items-center gap-2 justify-center flex-wrap">
                <div className="bg-emerald-500/20 px-2 py-1 rounded text-xs">Chunk 1: rows 1-2048</div>
                <div className="text-[var(--ink-subtle)]">|</div>
              ...
              <DiagramContainer title="Сценарий Collision" color="rose" className="h-full">
                <div className="text-xs text-[var(--ink-default)] space-y-2">
                  <div className="flex items-start gap-2">
                    <span className="text-[var(--ink-muted)]">T1:</span>
                    <span>Snapshot читает row id=500 со значением A</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="text-[var(--ink-muted)]">T2:</span>
          ...
          <table className="text-xs text-[var(--ink-default)] w-full">
            ...
            <tbody>
              <tr className="border-b border-[var(--line-thin)]">
                ...
                <td className="py-2 text-[var(--ink-muted)]">Медленнее, меньше memory</td>
              </tr>
              <tr className="border-b border-[var(--line-thin)]">
                ...
                <td className="py-2 text-[var(--ink-muted)]">Balanced performance</td>
              </tr>
              <tr className="border-b border-[var(--line-thin)]">
                <td className="py-2 pr-4 font-mono">4096</td>
                <td className="py-2 pr-4">Узкие rows (5-10 columns)</td>
                <td className="py-2 text-[var(--ink-muted)]">Быстрее, больше memory</td>
              </tr>
              <tr>
                <td className="py-2 pr-4 font-mono">8192</td>
                <td className="py-2 pr-4">Large tables, high-perf</td>
                <td className="py-2 text-[var(--ink-muted)]">Может вызвать memory pressure</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div className="space-y-6">
      ...
      <DiagramContainer title="Signal Table Schema" color="purple">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="font-mono text-xs bg-[var(--bg-sunken)] p-4 rounded w-full max-w-lg">
            <div className="text-purple-400 mb-2">CREATE TABLE debezium_signal (</div>
            <div className="pl-4 space-y-1">
              <div>
                <span className="text-emerald-400">id</span>
                <span className="text-[var(--ink-muted)]"> VARCHAR(36) NOT NULL,</span>
                <span className="text-[var(--ink-subtle)] ml-2">-- UUID</span>
              </div>
              <div>
                <span className="text-amber-400">type</span>
                <span className="text-[var(--ink-muted)]"> VARCHAR(32) NOT NULL,</span>
                <span className="text-[var(--ink-subtle)] ml-2">-- execute-snapshot</span>
              </div>
              <div>
                <span className="text-blue-400">data</span>
                <span className="text-[var(--ink-muted)]"> TEXT,</span>
                <span className="text-[var(--ink-subtle)] ml-2">-- JSON payload</span>
              </div>
              <div>
                <span className="text-rose-400">PRIMARY KEY (id)</span>
                <span className="text-[var(--ink-subtle)] ml-2">-- CRITICAL!</span>
              </div>
            </div>
      ...
      <DiagramContainer title="Signal Mechanism" color="amber">
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="text-xs text-[var(--ink-muted)]">Engineer</div>
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="text-xs text-[var(--ink-muted)]">MySQL</div>
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="text-xs text-[var(--ink-muted)]">Debezium</div>
          </div>
        </div>
      </DiagramContainer>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <DiagramContainer title="Execute Snapshot" color="emerald" className="flex-1">
          <div className="font-mono text-xs bg-[var(--bg-sunken)] p-3 rounded">
            <div className="text-[var(--ink-muted)]">INSERT INTO debezium_signal</div>
            <div className="text-[var(--ink-muted)]">(id, type, data) VALUES (</div>
            <div className="pl-2">
      ...
      <DiagramContainer title="CRITICAL: PRIMARY KEY обязателен" color="rose">
        <div className="text-sm text-[var(--ink-default)] space-y-2">
          ...
          <div className="text-xs text-[var(--ink-muted)]">
            ...
            <code className="bg-[var(--bg-sunken)] px-1 rounded">SHOW CREATE TABLE debezium_signal\G</code>
            <br />
            <span className="text-emerald-400">Исправление:</span>{' '}
            <code className="bg-[var(--bg-sunken)] px-1 rounded">ALTER TABLE debezium_signal ADD PRIMARY KEY (id);</code>
          </div>
        </div>
      </DiagramContainer>
      ...
      <DiagramContainer title="Connector Configuration" color="neutral">
        <div className="font-mono text-xs bg-[var(--bg-sunken)] p-3 rounded">
          <div className="text-[var(--ink-muted)]">// Обязательные свойства для signal table</div>
          <div className="mt-2">
          ...
          <div className="mt-3 text-[var(--ink-muted)]">
