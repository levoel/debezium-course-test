    ...
    <div class="space-y-6">
      <DiagramContainer title="gh-ost: Binlog-based Schema Change" color="emerald">
        <div class="flex flex-col items-center gap-4">
            ...
            <div class="flex flex-col items-center gap-2">
              <DiagramTooltip content="Оригинальная таблица orders. Приложение продолжает писать/читать во время миграции.">
                <FlowNode variant="database" tabIndex={0}>
                  orders
                  <br />
                  <span class="text-xs text-[var(--ink-muted)]">(original)</span>
                </FlowNode>
              </DiagramTooltip>
              <div class="text-xs text-emerald-400">Application writes</div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              <DiagramTooltip content="Ghost таблица с новой схемой. gh-ost копирует данные порциями и синхронизирует через binlog.">
                <FlowNode variant="database" tabIndex={0} className="border-2 border-purple-400">
                  _orders_gho
                  <br />
                  <span class="text-xs text-purple-400">(new schema)</span>
                </FlowNode>
              </DiagramTooltip>
              <div class="text-xs text-purple-400">+ status column</div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              <DiagramTooltip content="Changelog таблица для отслеживания прогресса миграции. Содержит метаданные о процессе.">
                <FlowNode variant="app" tabIndex={0} size="sm">
                  _orders_ghc
                  <br />
                  <span class="text-xs text-[var(--ink-muted)]">(metadata)</span>
                </FlowNode>
              </DiagramTooltip>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-emerald-400 font-bold">1. Copy Rows</div>
              <DiagramTooltip content="Chunked copy: SELECT * WHERE id >= N AND id < N+1000. Не блокирует таблицу.">
                <FlowNode variant="app" tabIndex={0} size="sm">
                  chunk copy
                  <br />
                  <span class="text-xs text-[var(--ink-muted)]">1000 rows</span>
                </FlowNode>
              </DiagramTooltip>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-blue-400 font-bold">2. Read Binlog</div>
              <DiagramTooltip content="Параллельно: gh-ost читает binlog для обнаружения изменений в orders. Применяет к ghost table.">
                <FlowNode variant="cluster" tabIndex={0} size="sm">
                  binlog events
                  <br />
                  <span class="text-xs text-[var(--ink-muted)]">INS/UPD/DEL</span>
                </FlowNode>
              </DiagramTooltip>
            </div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-purple-400 font-bold">3. Apply Changes</div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-amber-400 font-bold">4. Cutover</div>
      ...
      <DiagramContainer title="Debezium: Filter gh-ost Tables" color="amber">
        <div class="text-sm text-[var(--ink-default)] space-y-2">
          <div class="flex items-center gap-2">
            <span class="text-amber-400 font-bold">ВАЖНО:</span>
            <span>gh-ost создает временные таблицы, которые Debezium видит в binlog.</span>
          </div>
          <div class="font-mono text-xs bg-[var(--bg-sunken)] p-3 rounded mt-2">
            <div class="text-[var(--ink-muted)]">// Filter SMT для исключения helper tables</div>
            <div class="text-emerald-400">"transforms.Filter.condition":</div>
            <div class="text-amber-400 ml-2">"!value.source.table.matches('.*_(gho|ghc)$')"</div>
          </div>
          <div class="text-xs text-[var(--ink-muted)] mt-2">
  ...
  return (
    <div class="space-y-6">
        ...
        <div class="flex flex-col items-center gap-4">
          ...
          <div class="flex flex-col md:flex-row items-center gap-6 w-full justify-center">
            <div class="flex flex-col items-center gap-2">
              ...
              <div class="text-xs text-blue-400">+ TRIGGERS</div>
            </div>
          ...
          <div class="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <DiagramTooltip content="AFTER INSERT trigger: копирует новые строки в _orders_new.">
              <FlowNode variant="connector" tabindex={0} size="sm">
                AFTER INSERT
                <br />
                <span class="text-xs text-[var(--ink-muted)]">→ INSERT _orders_new</span>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="AFTER UPDATE trigger: обновляет строки в _orders_new.">
              <FlowNode variant="connector" tabIndex={0} size="sm">
                AFTER UPDATE
                <br />
                <span class="text-xs text-[var(--ink-muted)]">→ REPLACE _orders_new</span>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="AFTER DELETE trigger: удаляет строки из _orders_new.">
              <FlowNode variant="connector" tabindex={0} size="sm">
                AFTER DELETE
                <br />
                <span class="text-xs text-[var(--ink-muted)]">→ DELETE _orders_new</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
          ...
          <div class="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-blue-400 font-bold">1. Create + Triggers</div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-emerald-400 font-bold">2. Copy Chunks</div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-amber-400 font-bold">3. Rename</div>
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="text-xs text-rose-400 font-bold">4. Cleanup</div>
      ...
      <DiagramContainer title="pt-osc Advantage: Foreign Keys" color="emerald">
        <div class="text-sm text-[var(--ink-default)] space-y-2">
          <div class="flex items-center gap-2">
            <span class="text-emerald-400 font-bold">ПОДДЕРЖКА FK:</span>
      ...
      <DiagramContainer title="Debezium: Filter pt-osc Tables" color="amber">
        <div class="font-mono text-xs bg-[var(--bg-sunken)] p-3 rounded">
          <div class="text-[var(--ink-muted)]">// Filter SMT для pt-osc helper tables</div>
          <div class="text-emerald-400">"transforms.Filter.condition":</div>
          <div class="text-amber-400 ml-2">"!value.source.table.matches('.*_(new|old)$')"</div>
        </div>
      </DiagramContainer>
    </div>
  );
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <DiagramContainer title="gh-ost" color="emerald" className="flex-1" recommended>
          <div class="flex flex-col gap-4">
            ...
            <div class="text-xs text-emerald-400 font-medium mt-2">
              Рекомендуется для CDC
              <br />
              <span class="text-[var(--ink-muted)]">(если нет FK)</span>
            </div>
          </div>
        </DiagramContainer>
          ...
          <table class="text-xs text-[var(--ink-default)] w-full">
            <thead>
              <tr class="border-b border-[var(--line-thin)]">
                ...
                <th class="text-left py-2 pr-4">gh-ost</th>
                <th class="text-left py-2">pt-osc</th>
              </tr>
            </thead>
            <tbody>
              <tr class="border-b border-[var(--line-thin)]">
                <td class="py-2 pr-4">Mechanism</td>
                <td class="py-2 pr-4 text-emerald-400">Binlog reading</td>
                <td class="py-2 text-blue-400">Triggers</td>
              </tr>
              <tr class="border-b border-[var(--line-thin)]">
                <td class="py-2 pr-4">Helper tables</td>
                <td class="py-2 pr-4 font-mono">_gho, _ghc</td>
                <td class="py-2 font-mono">_new, _old</td>
              </tr>
              <tr class="border-b border-[var(--line-thin)]">
                <td class="py-2 pr-4">Foreign Keys</td>
                <td class="py-2 pr-4 text-rose-400">Not supported</td>
                <td class="py-2 text-emerald-400">Supported</td>
              </tr>
              <tr class="border-b border-[var(--line-thin)]">
                <td class="py-2 pr-4">Binlog impact</td>
                <td class="py-2 pr-4 text-amber-400">2x read load</td>
                <td class="py-2 text-emerald-400">Normal</td>
              </tr>
              <tr>
                <td class="py-2 pr-4">Pausable</td>
                <td class="py-2 pr-4 text-emerald-400">Yes</td>
                <td class="py-2 text-amber-400">Requires cleanup</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  return (
    <div class="space-y-6">
      <DiagramContainer title="Ghost Table Lifecycle (gh-ost)" color="purple">
        <div class="flex flex-col items-center gap-4">
          ...
          <div class="relative w-full">
            {/* Timeline line */}
            <div class="absolute left-0 right-0 top-1/2 h-0.5 bg-[var(--bg-deep)] transform -translate-y-1/2" />
              ...
              <div class="flex flex-col items-center gap-2 z-10">
                ...
                <DiagramTooltip content="Step 1: gh-ost создает _orders_gho с новой схемой и _orders_ghc для метаданных.">
                  <FlowNode variant="connector" tabindex={0} size="sm">
                    CREATE
                    <br />
                    <span class="text-xs text-emerald-400">_orders_gho</span>
                  </FlowNode>
                </DiagramTooltip>
                <div class="text-xs text-[var(--ink-muted)]">T0</div>
              </div>
              ...
              <div class="flex flex-col items-center gap-2 z-10">
                ...
                <DiagramTooltip content="Step 2: Chunked copy данных из orders в _orders_gho. Параллельно binlog sync.">
                  <FlowNode variant="cluster" tabindex={0} size="sm">
                    COPY
                    <br />
                    <span class="text-xs text-blue-400">data migration</span>
                  </FlowNode>
                </DiagramTooltip>
                <div class="text-xs text-[var(--ink-muted)]">T1-Tn</div>
              </div>

              {/* Step 3: Cutover */}
              <div class="flex flex-col items-center gap-2 z-10">
                <div class="w-4 h-4 bg-amber-400 rounded-full" />
                <DiagramTooltip content="Step 3: Atomic rename. orders → _orders_del (temp), _orders_gho → orders. Milliseconds lock.">
                  <FlowNode variant="app" tabindex={0} size="sm" className="border-2 border-amber-400">
                    RENAME
                    <br />
                    <span class="text-xs text-amber-400">cutover</span>
                  </FlowNode>
                </DiagramTooltip>
                <div class="text-xs text-[var(--ink-muted)]">Tn+1</div>
              </div>

              {/* Step 4: Cleanup */}
              <div class="flex flex-col items-center gap-2 z-10">
                <div class="w-4 h-4 bg-rose-400 rounded-full" />
                <DiagramTooltip content="Step 4: Удаление старой таблицы и _orders_ghc. Cleanup завершён.">
                  <FlowNode variant="sink" tabindex={0} size="sm">
                    DROP
                    <br />
                    <span class="text-xs text-rose-400">cleanup</span>
                  </FlowNode>
                </DiagramTooltip>
                <div class="text-xs text-[var(--ink-muted)]">Tn+2</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <DiagramContainer title="Debezium Perspective" color="amber">
        <div class="flex flex-col gap-4">
          <div class="text-sm text-[var(--ink-default)]">
            <span class="text-amber-400 font-bold">Debezium видит в binlog:</span>
          </div>

          <div class="flex flex-col md:flex-row gap-4">
            <div class="flex-1 bg-[var(--bg-sunken)] p-3 rounded">
              <div class="text-xs text-emerald-400 font-bold mb-2">T0: CREATE</div>
              <div class="text-xs text-[var(--ink-default)] space-y-1">
                <div>CREATE TABLE _orders_gho (...)</div>
                <div>CREATE TABLE _orders_ghc (...)</div>
              </div>
              <div class="text-xs text-amber-400 mt-2">→ Schema history updated</div>
            </div>

            <div class="flex-1 bg-[var(--bg-sunken)] p-3 rounded">
              <div class="text-xs text-blue-400 font-bold mb-2">T1-Tn: DATA</div>
              <div class="text-xs text-[var(--ink-default)] space-y-1">
                <div>INSERT _orders_gho (copy)</div>
                <div>INSERT/UPDATE/DELETE orders</div>
                <div>INSERT _orders_gho (sync)</div>
              </div>
              <div class="text-xs text-emerald-400 mt-2">→ Filter SMT excludes _gho</div>
            </div>

            <div class="flex-1 bg-[var(--bg-sunken)] p-3 rounded">
              <div class="text-xs text-amber-400 font-bold mb-2">Tn+1: RENAME</div>
              <div class="text-xs text-[var(--ink-default)] space-y-1">
                <div>RENAME orders → _orders_old</div>
                <div>RENAME _orders_gho → orders</div>
              </div>
              <div class="text-xs text-purple-400 mt-2">→ New schema active</div>
            </div>

            <div class="flex-1 bg-[var(--bg-sunken)] p-3 rounded">
              <div class="text-xs text-rose-400 font-bold mb-2">Tn+2: DROP</div>
              <div class="text-xs text-[var(--ink-default)] space-y-1">
                <div>DROP _orders_old</div>
                <div>DROP _orders_ghc</div>
              </div>
              <div class="text-xs text-[var(--ink-muted)] mt-2">→ Schema history updated</div>
            </div>
          </div>
        </div>
      </DiagramContainer>
      ...
      <DiagramContainer title="Consumer Impact" color="emerald">
        <div class="text-sm text-[var(--ink-default)] space-y-2">
          <div class="flex items-center gap-2">
            <span class="text-emerald-400 font-bold">РЕЗУЛЬТАТ:</span>
          ...
          <div class="text-xs text-[var(--ink-muted)]">
            ...
            <br />
            <br />
            <span class="text-amber-400">Важно:</span> Consumers должны быть готовы к schema evolution
            (новые optional поля).
          </div>
        </div>
      </DiagramContainer>
