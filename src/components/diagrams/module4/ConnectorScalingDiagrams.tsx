    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      >
        <div class="flex flex-col gap-4">
          ...
          <div class="text-center text-rose-400 text-xs">
      ...
      >
        <div class="flex flex-col gap-4 items-center">
          ...
          <DiagramTooltip content="Единственный task читает WAL последовательно. tasks.max игнорируется для PostgreSQL, MySQL и Oracle коннекторов.">
            <FlowNode variant="connector" tabIndex={0} size="lg">
              <div class="text-center">
                <div>Task 1</div>
                <div class="text-xs text-[var(--ink-muted)]">(ЕДИНСТВЕННЫЙ)</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <div class="bg-amber-500/20 border border-amber-400/30 rounded-lg px-3 py-2 text-center">
            <DiagramTooltip content="Параметр tasks.max полностью игнорируется. Можно установить 100, результат будет 1 task. Это не баг - это архитектурное ограничение.">
              <span class="text-amber-700 text-sm font-mono cursor-help">
  ...
  return (
    ...
    >
      <div class="flex flex-col gap-6">
        {/* WAL events sequence */}
        <div class="flex flex-col md:flex-row items-center justify-center gap-2 overflow-x-auto pb-2">
          <DiagramTooltip content="LSN (Log Sequence Number) - уникальная позиция в WAL. События должны обрабатываться строго по порядку LSN для транзакционной целостности.">
            <FlowNode variant="sink" tabIndex={0} size="sm">
              <div class="text-center">
                <span class="block text-xs text-[var(--ink-muted)]">Event 1</span>
                <span class="font-mono text-xs">LSN: 0/1000</span>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Каждое событие имеет уникальный LSN. Пропуск или изменение порядка нарушит целостность данных.">
            <FlowNode variant="sink" tabIndex={0} size="sm">
              <div class="text-center">
                <span class="block text-xs text-[var(--ink-muted)]">Event 2</span>
                <span class="font-mono text-xs">LSN: 0/1001</span>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Все изменения одной транзакции должны обрабатываться атомарно. Parallel processing нарушит это требование.">
            <FlowNode variant="sink" tabIndex={0} size="sm">
              <div class="text-center">
                <span class="block text-xs text-[var(--ink-muted)]">Event 3</span>
                <span class="font-mono text-xs">LSN: 0/1002</span>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <FlowNode variant="sink" tabIndex={0} size="sm">
            <div class="text-center">
              <span class="block text-xs text-[var(--ink-muted)]">Event 4</span>
              <span class="font-mono text-xs">LSN: 0/1003</span>
            </div>
          </FlowNode>
          ...
          <FlowNode variant="sink" tabIndex={0} size="sm">
            <div class="text-center">
              <span class="block text-xs text-[var(--ink-muted)]">Event 5</span>
              <span class="font-mono text-xs">LSN: 0/1004</span>
            </div>
          </FlowNode>
        </div>
        ...
        <div class="flex flex-col items-center gap-3">
          <DiagramTooltip content="Replication slot отслеживает одну позицию (restart_lsn). Невозможно иметь несколько позиций для параллельного чтения.">
            <div class="bg-amber-500/20 border border-amber-400/30 rounded-xl px-4 py-3 text-center">
              <div class="text-amber-700 font-semibold">Replication Slot</div>
              <div class="text-xs text-[var(--ink-muted)] font-mono mt-1">position: 0/1002</div>
            </div>
          </DiagramTooltip>

          <div class="text-center text-sm text-[var(--ink-muted)] max-w-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
          <div class="bg-[var(--bg-sunken)] p-3 rounded-lg text-center">
            <div class="text-blue-400 font-semibold text-sm mb-1">Порядок имеет значение</div>
            <div class="text-xs text-[var(--ink-muted)]">События обрабатываются строго по LSN</div>
          </div>
          <div class="bg-[var(--bg-sunken)] p-3 rounded-lg text-center">
            <div class="text-purple-400 font-semibold text-sm mb-1">Одна позиция</div>
            <div class="text-xs text-[var(--ink-muted)]">Slot отслеживает один restart_lsn</div>
          </div>
          <div class="bg-[var(--bg-sunken)] p-3 rounded-lg text-center">
            <div class="text-emerald-400 font-semibold text-sm mb-1">Транзакционность</div>
            <div class="text-xs text-[var(--ink-muted)]">Атомарная обработка транзакций</div>
          </div>
        </div>
      </div>
    </DiagramContainer>
      ...
      <div class="flex flex-col items-center gap-4">
        ...
        <DiagramTooltip content="PostgreSQL отдает WAL через logical decoding. Один replication slot = один поток данных.">
          <FlowNode variant="database" tabindex={0} size="lg">
            <div class="text-center">
              <div>PostgreSQL</div>
              <div class="text-xs text-[var(--ink-muted)]">WAL Stream</div>
            </div>
          </FlowNode>
        </DiagramTooltip>
        ...
        <div class="bg-[var(--bg-sunken)] border border-[var(--line-thin)] rounded-2xl p-4 w-full max-w-md">
          <div class="text-center text-sm text-[var(--ink-default)] mb-4 font-semibold">
          ...
          <div class="flex flex-col gap-3">
            <DiagramTooltip content="WAL Reader - BOTTLENECK! Один поток читает WAL последовательно. Это физическое ограничение, которое нельзя обойти через конфигурацию.">
              ...
              >
                <div class="text-center">
                  <div>WAL Reader</div>
                  <div class="text-xs text-rose-700">(1 поток - BOTTLENECK)</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="Internal Queue буферизует события перед отправкой в Kafka. По умолчанию 8192 события, можно увеличить до 16384+.">
              <FlowNode variant="connector" tabIndex={0}>
                <div class="text-center">
                  ...
                  <div class="text-xs text-[var(--ink-muted)]">(8192 events)</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
            ...
            <DiagramTooltip content="Kafka Writer отправляет батчи событий в Kafka. Можно оптимизировать через producer.override.* параметры.">
              <FlowNode variant="sink" tabIndex={0}>
                <div class="text-center">
                  ...
                  <div class="text-xs text-[var(--ink-muted)]">(1 поток)</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
        ...
        <DiagramTooltip content="Kafka получает события от одного producer. Throughput ограничен скоростью WAL Reader.">
          <FlowNode variant="cluster" tabIndex={0} size="lg">
            <div class="text-center">
              <div>Kafka</div>
              <div class="text-xs text-[var(--ink-muted)]">Topic Partitions</div>
            </div>
          </FlowNode>
        </DiagramTooltip>

        {/* Note */}
        <div class="bg-rose-500/10 border border-rose-400/30 rounded-lg px-4 py-2 text-center mt-2">
          <span class="text-rose-700 text-sm">
  ...
  return (
    ...
    >
      <div class="flex flex-col lg:flex-row gap-6 items-start">
        {/* PostgreSQL tables */}
        <div class="flex flex-col gap-3">
          <div class="text-center text-sm text-[var(--ink-muted)] mb-2">PostgreSQL</div>
          <DiagramTooltip content="Таблицы orders и order_items - домен заказов. Отдельный коннектор и slot для изоляции.">
            <FlowNode variant="sink" tabindex={0} size="sm">
              <div class="text-center">
                <div class="text-blue-700">orders</div>
                <div class="text-xs text-[var(--ink-muted)]">order_items</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Таблицы products и inventory - домен инвентаря. Независимый от заказов.">
            <FlowNode variant="database" tabindex={0} size="sm">
              <div class="text-center">
                <div class="text-purple-700">products</div>
                <div class="text-xs text-[var(--ink-muted)]">inventory</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Таблицы customers и addresses - домен клиентов. Отдельные потребители.">
            <FlowNode variant="connector" tabindex={0} size="sm">
              <div class="text-center">
                <div class="text-emerald-700">customers</div>
                <div class="text-xs text-[var(--ink-muted)]">addresses</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
        </div>
        ...
        {/* Connectors */}
        <div class="flex flex-col gap-3">
          <div class="text-center text-sm text-[var(--ink-muted)] mb-2">Connectors</div>
          <DiagramTooltip content="orders-connector с отдельным slot. Failure не влияет на другие домены.">
            <FlowNode variant="sink" tabIndex={0} size="sm">
              <div class="text-center">
                <div class="text-blue-700">orders-connector</div>
                <div class="text-xs text-[var(--ink-muted)]">task=1</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="inventory-connector с отдельным slot. Независимый offset tracking.">
            <FlowNode variant="database" tabIndex={0} size="sm">
              <div class="text-center">
                <div class="text-purple-700">inventory-connector</div>
                <div class="text-xs text-[var(--ink-muted)]">task=1</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="customers-connector с отдельным slot. Можно мониторить lag per-domain.">
            <FlowNode variant="connector" tabindex={0} size="sm">
              <div class="text-center">
                <div class="text-emerald-700">customers-connector</div>
                <div class="text-xs text-[var(--ink-muted)]">task=1</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
        ...
        <div class="flex flex-col gap-3">
          ...
          <DiagramTooltip content="inventory.public.* - топики для инвентаря. Независимые retention и partitioning.">
            <FlowNode variant="database" tabIndex={0} size="sm">
              <div class="text-center text-xs">
                <div class="text-purple-700">inventory.public.*</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="customers.public.* - топики для клиентов. Можно настроить разные SLO.">
            <FlowNode variant="connector" tabIndex={0} size="sm">
              <div class="text-center text-xs">
                <div class="text-emerald-700">customers.public.*</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        <div class="bg-emerald-500/10 border border-emerald-400/30 rounded-lg p-3">
          <div class="text-emerald-700 font-semibold text-sm mb-2">Преимущества</div>
        ...
        <div class="bg-rose-500/10 border border-rose-400/30 rounded-lg p-3">
          <div class="text-rose-700 font-semibold text-sm mb-2">Недостатки</div>
          <ul class="text-xs text-[var(--ink-default)] space-y-1">
  ...
  return (
    ...
    >
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col lg:flex-row items-center justify-center gap-4">
          ...
          <DiagramTooltip content="Таблица orders - все данные из одного источника.">
            <FlowNode variant="database" tabIndex={0}>
              <div class="text-center">
                <div>PostgreSQL</div>
                <div class="text-xs text-[var(--ink-muted)]">orders table</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <DiagramTooltip content="Топик с 8 партициями. Key = primary key таблицы. События одного ключа всегда в одной партиции (ordering гарантирован).">
            <div class="bg-amber-500/20 border border-amber-400/30 rounded-xl px-4 py-3">
              <div class="text-center">
                <div class="text-amber-700 font-semibold">orders topic</div>
                <div class="text-xs text-[var(--ink-muted)]">8 partitions</div>
              </div>
            </div>
          </DiagramTooltip>
          ...
          <div class="flex flex-col gap-2">
            <DiagramTooltip content="Consumer 1 обрабатывает партиции 0-1. Параллельно с другими consumers.">
              <FlowNode variant="app" tabindex={0} size="sm">
                <div class="text-center text-xs">
                  <div>Consumer 1</div>
                  <div class="text-[var(--ink-muted)]">p: 0-1</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
            <DiagramTooltip content="Consumer 2 обрабатывает партиции 2-3.">
              <FlowNode variant="app" tabIndex={0} size="sm">
                <div class="text-center text-xs">
                  <div>Consumer 2</div>
                  <div class="text-[var(--ink-muted)]">p: 2-3</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
            <DiagramTooltip content="Consumer 3 обрабатывает партиции 4-5.">
              <FlowNode variant="app" tabIndex={0} size="sm">
                <div class="text-center text-xs">
                  <div>Consumer 3</div>
                  <div class="text-[var(--ink-muted)]">p: 4-5</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
            <DiagramTooltip content="Consumer 4 обрабатывает партиции 6-7.">
              <FlowNode variant="app" tabIndex={0} size="sm">
                <div class="text-center text-xs">
                  <div>Consumer 4</div>
                  <div class="text-[var(--ink-muted)]">p: 6-7</div>
                </div>
              </FlowNode>
            </DiagramTooltip>
          </div>
        ...
        {/* How it works */}
        <div class="bg-[var(--bg-sunken)] rounded-lg p-4">
          <div class="text-sm text-[var(--ink-default)] font-semibold mb-3">Как это работает:</div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-[var(--ink-muted)]">
            ...
            <div class="flex items-start gap-2">
              <span class="text-emerald-400 font-semibold">4.</span>
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col items-center gap-4">
          ...
          <DiagramTooltip content="Первый шаг - оценить текущий throughput. Performance ceiling одного коннектора ~7,000 events/sec.">
            <FlowNode variant="connector" tabindex={0} size="lg">
              <div class="text-center">
                ...
                <div class="text-xs text-[var(--ink-muted)]">Текущий vs целевой</div>
              </div>
            </FlowNode>
          </DiagramTooltip>
          ...
          <div class="flex flex-col md:flex-row gap-6 items-start">
            {/* Left: Under ceiling */}
            <div class="flex flex-col items-center gap-3">
              <div class="text-sm text-[var(--ink-muted)]">Меньше 7K/sec</div>
              <Arrow direction="down" />
              <DiagramTooltip content="Performance Tuning: max.queue.size=16384, max.batch.size=4096, producer.override.compression.type=lz4. Максимизирует throughput одного коннектора.">
                <FlowNode variant="sink" tabindex={0}>
                  <div class="text-center">
                    <div class="text-blue-700">Стратегия 3:</div>
                    <div class="text-sm">Performance Tuning</div>
                  </div>
                </FlowNode>
              </DiagramTooltip>
            ...
            <div class="flex flex-col items-center gap-3">
              <div class="text-sm text-[var(--ink-muted)]">~7K/sec или больше</div>
              <Arrow direction="down" />
              ...
              <div class="flex flex-row gap-6 mt-3">
                ...
                <div class="flex flex-col items-center gap-2">
                  ...
                  <DiagramTooltip content="Multiple Connectors: разделить таблицы по доменам, отдельные slots и offsets. Горизонтальное масштабирование.">
                    <FlowNode variant="database" tabindex={0}>
                      <div class="text-center">
                        <div class="text-purple-700">Стратегия 1:</div>
                        <div class="text-sm">Multiple Connectors</div>
                      </div>
                    </FlowNode>
                  </DiagramTooltip>
                </div>
                ...
                <div class="flex flex-col items-center gap-2">
                  ...
                  <DiagramTooltip content="Если таблицы связаны, проверьте требования к ordering. Per-key ordering обычно достаточен.">
                    <FlowNode variant="connector" tabindex={0} size="sm">
                      <div class="text-center">
                        <div class="text-xs">Нужен ordering</div>
                        <div class="text-xs">per-key?</div>
                      </div>
                    </FlowNode>
                  </DiagramTooltip>
                  ...
                  <div class="flex flex-row gap-4 mt-2">
                    <div class="flex flex-col items-center gap-2">
                      ...
                      <DiagramTooltip content="Downstream Parallelization: один коннектор, много партиций Kafka, параллельные consumers. Ordering per-key гарантирован.">
                        <FlowNode variant="app" tabindex={0} size="sm">
                          <div class="text-center text-xs">
                            <div class="text-emerald-700">Стратегия 2:</div>
                            <div>Downstream</div>
                          </div>
                        </FlowNode>
                      </DiagramTooltip>
                    ...
                    <div class="flex flex-col items-center gap-2">
                      <div class="text-xs text-amber-400">Нет</div>
                      <DiagramTooltip content="Комбинация стратегий: Multiple Connectors + Downstream Parallelization для максимального throughput.">
                        <FlowNode variant="target" tabIndex={0} size="sm">
                          <div class="text-center text-xs">
                            <div class="text-amber-700">Комбинация:</div>
                            <div>Multiple + DS</div>
                          </div>
                        </FlowNode>
                      </DiagramTooltip>
        ...
        {/* Performance note */}
        <div class="bg-amber-500/10 border border-amber-400/30 rounded-lg px-4 py-3 text-center">
