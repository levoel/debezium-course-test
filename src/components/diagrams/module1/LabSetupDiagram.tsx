  ...
  return (
    <div class="flex flex-col gap-6">
      ...
      <div class="text-center">
        <h3 class="text-lg font-semibold text-[var(--ink-default)]">
          ...
          <div class="flex flex-col items-center gap-4">
            <DiagramTooltip content="База данных с wal_level=logical и настроенными replication slots. Порт 5433 (не 5432!) чтобы избежать конфликта с локальной установкой PostgreSQL.">
              <FlowNode variant="database" tabIndex={0}>
                PostgreSQL
                <br />
                <span class="text-xs opacity-75">:5433</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
        </DiagramContainer>
          ...
          <div class="flex flex-col items-center gap-4">
            ...
            <DiagramTooltip content="Kafka Connect с установленным Debezium PostgreSQL коннектором. REST API на порту 8083 для управления.">
              <FlowNode variant="connector" tabIndex={0}>
                Debezium Connect
                <br />
                <span class="text-xs opacity-75">:8083</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
        </DiagramContainer>
            ...
            <div class="flex flex-wrap justify-center gap-3">
              <DiagramTooltip content="Сбор метрик из Kafka Connect через JMX. Хранит time-series данные для мониторинга CDC pipeline.">
                <FlowNode variant="sink" tabIndex={0} size="sm">
                  Prometheus
                  <br />
                  <span class="text-xs opacity-75">:9090</span>
                </FlowNode>
              </DiagramTooltip>
              ...
              <DiagramTooltip content="Дашборды для визуализации метрик. Логин: admin/admin. Подключен к Prometheus как data source.">
                <FlowNode variant="app" tabIndex={0} size="sm">
                  Grafana
                  <br />
                  <span class="text-xs opacity-75">:3000</span>
                </FlowNode>
              </DiagramTooltip>
            </div>
          </div>
          ...
          <div class="flex flex-col items-center gap-4">
            <DiagramTooltip content="Python окружение для экспериментов. confluent-kafka библиотека уже установлена. Подключается к Kafka как kafka:9092 (внутренняя сеть Docker).">
              <FlowNode variant="app" tabIndex={0}>
                JupyterLab
                <br />
                <span class="text-xs opacity-75">:8888</span>
              </FlowNode>
            </DiagramTooltip>
          </div>
        </DiagramContainer>
      ...
      {/* Data Flow Legend */}
      <div class="mt-4 p-4 rounded-lg bg-[var(--bg-deep)] border border-[var(--line-medium)]">
        <h4 class="text-sm font-semibold text-[var(--ink-default)] mb-3">
          Основные потоки данных:
        </h4>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-[var(--ink-muted)]">
      ...
      <div class="p-4 rounded-lg bg-emerald-500/10 border border-emerald-400/30">
        <h4 class="text-sm font-semibold text-emerald-700 mb-2">
          Быстрый старт:
        </h4>
        <ul class="text-xs text-[var(--ink-default)] space-y-1">
          <li><code class="text-emerald-400">docker compose up -d</code> — запустить всё окружение</li>
          <li><code class="text-emerald-400">localhost:8888</code> — JupyterLab для упражнений</li>
          <li><code class="text-emerald-400">localhost:8083</code> — Kafka Connect REST API</li>
          <li><code class="text-emerald-400">localhost:3000</code> — Grafana мониторинг</li>
        </ul>
      </div>
    </div>
  );
