...
> **<Icon name="warning" className="text-yellow-400 inline" size="1.2em" /> КРИТИЧЕСКОЕ ОБНОВЛЕНИЕ**
>
...
**Теперь (pandas 3.0+):**
```python
# [NO] Chained assignment НЕ РАБОТАЕТ (silent failure)
df[df['total'] > 100]['status'] = 'high-value'

# [OK] Правильный способ
...
|----------|------|----------|---------|-------|
| **Snapshot** | `'r'` | null | Да | Начальная загрузка существующих данных |
| **INSERT** | `'c'` | null | Да | Новая запись добавлена |
| **UPDATE** | `'u'` | Да | Да | Запись изменена |
| **DELETE** | `'d'` | Да | null | Запись удалена |
...
consumer.close()
print(f"[OK] Собрано {len(messages)} сообщений\n")
