...
<PyflinkVsPysparkComparisonDiagram client:load />
...
| **Exactly-once в Kafka** | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Native support | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> At-least-once only |
| **Интеграция с ML** | Limited | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> MLlib, feature stores |
| **Batch + Stream unified** | Separate APIs | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Same DataFrame API |
| **Data lake writes** | Possible | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Native (Parquet, Delta) |
| **Complex event patterns** | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> CEP library | Basic windowing only |
...
<StructuredStreamingConceptDiagram client:visible />
...
<PysparkWatermarkDiagram client:visible />
...
<MicroBatchVsContinuousDiagram client:visible />
