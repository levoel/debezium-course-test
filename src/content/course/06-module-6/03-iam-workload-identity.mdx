...
<Mermaid chart={`
graph LR
    Pod[Pod в GKE] --> KSA[Kubernetes<br/>Service Account]
    KSA --> |iam.gke.io/gcp-service-account| WI[Workload Identity<br/>Binding]
    WI --> GCPSA[GCP Service Account]
    GCPSA --> IAM[IAM Roles]
    IAM --> PubSub[Pub/Sub API]
    IAM --> SM[Secret Manager API]

    style Pod fill:#e3f2fd
    style GCPSA fill:#c8e6c9
