...
<KnowledgeCheck
  question={'Почему Debezium Server должен иметь роль roles/pubsub.publisher, а не более широкую roles/pubsub.admin или roles/editor?'}
  answer={'Principle of least privilege: Debezium Server только публикует события в Pub/Sub topics — ему не нужно создавать/удалять топики или управлять подписками. roles/pubsub.publisher ограничивает blast radius — если Debezium Server скомпрометирован, атакующий может только публиковать сообщения, но не получить доступ к другим ресурсам проекта. roles/editor предоставляет почти полный доступ к GCP проекту.'}
/>
