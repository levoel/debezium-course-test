...
<KnowledgeCheck
  question={'Debezium Server экспортирует метрику MilliSecondsBehindSource. Что именно она измеряет и почему это ключевая метрика для CDC pipeline?'}
  answer={'MilliSecondsBehindSource измеряет задержку между timestamp события в PostgreSQL WAL и моментом его обработки Debezium Server. Это показывает, насколько CDC pipeline отстаёт от реального времени. Ключевая метрика, потому что: (1) напрямую отражает freshness данных в downstream системах (BigQuery, Cloud Run), (2) рост lag предупреждает о bottleneck до его влияния на бизнес, (3) связана с WAL retention — длительный lag приводит к WAL bloat на Cloud SQL.'}
/>
...
<KnowledgeCheck
  question={'Pub/Sub метрика oldest_unacked_message_age показывает 600 секунд (10 минут). Что это означает для CDC pipeline и каков порядок диагностики?'}
  answer={'Oldest_unacked_message_age = 600 секунд означает, что самое старое необработанное сообщение ожидает в subscription 10 минут — consumers (Dataflow/Cloud Run) не успевают обрабатывать события. Диагностика: (1) проверить Dataflow job status и system_lag, (2) проверить Cloud Run error rate (5xx), (3) проверить dead_letter_message_count, (4) если consumer здоров — увеличить Dataflow workers или Cloud Run max-instances. Растущий backlog при здоровых consumers указывает на недостаток ресурсов.'}
/>
...
```mermaid
graph TD
    subgraph Row1["Row 1: High-Level Health"]
        CS["Cloud SQL Status"]
        DS["Debezium Status"]
        PS["Pub/Sub Status"]
        DFS["Dataflow Status"]
        CRS["Cloud Run Status"]
    end

    subgraph Row2["Row 2: Source Metrics (Cloud SQL)"]
        CPU["CPU Utilization"]
        DISK["Disk Utilization"]
        CONN["Active Connections"]
        WAL["WAL Size"]
        RSLAG["Replication Slot Lag"]
    end

    subgraph Row3["Row 3: CDC Engine (Debezium)"]
        MBS["MilliSecondsBehindSource"]
        EPS["Events Processed/sec"]
        QRC["Queue Remaining Capacity"]
    end

    subgraph Row4["Row 4: Messaging (Pub/Sub)"]
        OUMA["Oldest Unacked Message Age"]
        NUM["Num Undelivered Messages"]
        DLQ["Dead Letter Queue Count"]
    end

    subgraph Row5["Row 5: Consumers (Dataflow + Cloud Run)"]
        DFSL["Dataflow System Lag"]
        DFVC["Dataflow vCPUs"]
        CRRR["Cloud Run Request Rate"]
        CRER["Cloud Run Error Rate"]
    end
```

<MonitoringPointsHierarchyDiagram client:visible />
...
<AlertFlowDiagram client:visible />
