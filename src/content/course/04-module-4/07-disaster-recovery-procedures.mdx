...
import { FailureModesDiagram, StateStorageLocationsDiagram, OrphanedSlotCleanupDiagram } from '../../../components/diagrams/module4';
...
<FailureModesDiagram client:load />
...
<StateStorageLocationsDiagram client:visible />
...
<KnowledgeCheck
  question="Почему при одновременном сбое Kafka и коннектора Debezium необходимо восстановить Kafka первым?"
  answer="При запуске коннектор обращается к connect-offsets топику в Kafka для определения последней позиции (LSN). Без доступного Kafka коннектор не сможет определить, откуда продолжить чтение — он либо начнет с начала (дубли), либо упадет с ошибкой."
/>
...
<OrphanedSlotCleanupDiagram client:visible />
...
<KnowledgeCheck
  question="При случайном удалении коннектора через REST API, почему его можно восстановить без full resnapshot, пересоздав с тем же именем и topic.prefix?"
  answer="Offsets хранятся в Kafka топике connect-offsets и привязаны к имени коннектора и topic.prefix. При удалении коннектора offsets НЕ удаляются. Пересоздание коннектора с идентичным именем и конфигурацией автоматически подхватит сохраненные offsets и продолжит с последней позиции."
/>
