...
<KnowledgeCheck
  question="Почему для панели Current Lag используется Gauge, а не Stat? Какое преимущество дает Gauge для метрики отставания?"
  answer="Gauge визуально показывает текущее значение относительно шкалы с цветовыми порогами (зеленый/желтый/красный). Оператор мгновенно видит, в какой зоне находится lag — нормальной, предупредительной или критической — без необходимости интерпретировать числовое значение."
/>
...
<PanelRowLayoutDiagram client:visible />
...
<KnowledgeCheck
  question="Зачем на графике Lag Trend добавлять визуальные threshold lines на 5s и 30s, если алерты уже настроены на эти значения?"
  answer="Threshold lines на графике дают контекст при визуальном анализе: оператор видит, насколько близко lag подходил к порогам, даже если алерт не сработал. Это помогает при capacity planning и post-mortem — например, обнаружить тренд приближения к warning уровню."
/>
...
<HealthStatesComparisonDiagram client:visible />
