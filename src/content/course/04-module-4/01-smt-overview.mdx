...
- **Filter**: <1ms (простое условие Groovy)
- **ExtractNewRecordState**: 1-2ms (JSON processing)
- **MaskField**: <1ms (field replacement)
- **ByLogicalTableRouter**: <1ms (regex match)
...
│                      └─ НЕТ ──→ Overhead <10ms? ── ДА ──→ SMT подходит
│                                                    └─ НЕТ ──→ Kafka Streams
```
...
5. SMT подходит для **простых stateless операций** с overhead <10ms
