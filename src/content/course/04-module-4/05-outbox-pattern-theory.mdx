...
flowchart TB
    ...
    OK -->|"YES"| SUCCESS["✅ БД обновлена<br/>✅ Событие отправлено<br/>Consistency OK"]
    OK -->|"NO"| FAIL["✅ БД обновлена<br/>❌ Событие НЕ отправлено<br/>💥 Data loss!"]
...
<Mermaid chart={`
flowchart LR
    subgraph SCENARIO1["Нормальный flow"]
        A1["INSERT outbox"] --> B1["Debezium читает"] --> C1["Kafka receives"] --> D1["Debezium offset commit"]
    end

    subgraph SCENARIO2["Retry scenario"]
        A2["INSERT outbox"] --> B2["Debezium читает"] --> C2["Kafka receives"] --> D2["Debezium crash<br/>(offset не committed)"]
