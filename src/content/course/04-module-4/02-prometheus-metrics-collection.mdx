...
<Mermaid chart={`
flowchart LR
    subgraph CONNECT["Kafka Connect Container"]
        JVM["Debezium JVM"]
        JMX["JMX MBeans"]
        EXPORTER["JMX Exporter<br/>Agent на :9404"]
        JVM --> JMX
        JMX --> EXPORTER
    end

    subgraph PROM["Prometheus Container"]
        SCRAPER["Scraper<br/>каждые 15s"]
        TSDB["Time Series DB"]
        SCRAPER --> TSDB
    end

    EXPORTER -->|"GET /metrics"| SCRAPER
    ...
    style SCRAPER fill:#e11d48
    style TSDB fill:#8b5cf6
`} client:visible />
