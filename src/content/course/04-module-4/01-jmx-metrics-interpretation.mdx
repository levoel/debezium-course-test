...
<LagCalculationDiagram client:visible />
...
<StalenessScenariosDiagram client:visible />
...
<KnowledgeCheck
  question="Почему MilliSecondsBehindSource никогда не равен нулю, даже когда CDC pipeline работает идеально?"
  answer="Всегда существует processing delay: время на чтение WAL, сериализацию события и отправку в Kafka. Типичные значения 50-200ms при idle системе — это нормальный overhead, а не признак проблемы."
/>
...
<DiagnosticDecisionTreeDiagram client:visible />
...
<KnowledgeCheck
  question="Мониторинг показывает QueueRemainingCapacity = 100 из 8192, а MilliSecondsBehindSource растет. В какой части pipeline находится bottleneck — в чтении WAL или в записи в Kafka?"
  answer="Bottleneck в записи в Kafka. QueueRemainingCapacity близок к нулю означает, что очередь почти заполнена — WAL reader помещает события в очередь быстрее, чем Kafka writer их забирает. Нужно проверить Kafka broker latency и network."
/>

## Особенности метрик в Debezium 3.4

Наша lab-среда использует Debezium 3.4.0.Final (project standard). Что это даёт:

**Доступны per-table метрики (Debezium 3.0+).** Метрики можно фильтровать на уровне отдельной таблицы — вы можете увидеть lag и event counts отдельно для `orders` vs `customers`:

- `debezium_metrics_TotalNumberOfCreateEventsSeen{table="orders"}`
- `debezium_metrics_TotalNumberOfUpdateEventsSeen{table="orders"}`

В legacy-окружениях с Debezium 2.x per-table granularity недоступна — там приходилось разносить таблицы по отдельным коннекторам.
...
6. Debezium 3.4 поддерживает per-table метрики (появились в 3.0+) — фильтруйте `{table="..."}` для гранулярного мониторинга
