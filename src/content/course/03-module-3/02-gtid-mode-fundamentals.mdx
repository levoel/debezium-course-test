...
import { GtidAnatomyDiagram, GtidReplicationDiagram, GtidFailoverComparisonDiagram } from '../../../components/diagrams/module3/GtidDiagrams';
...
<GtidAnatomyDiagram client:visible />
...
<GtidReplicationDiagram client:visible />
...
<KnowledgeCheck
  question="Почему GTID критичен для Debezium при failover MySQL сервера?"
  answer="GTID глобально уникален на всех серверах в топологии репликации. При failover Debezium может продолжить чтение с нового primary, просто запросив транзакции после последнего обработанного GTID, без необходимости пересчитывать файловые позиции."
/>
...
<GtidFailoverComparisonDiagram client:visible />
...
<KnowledgeCheck
  question="Что произойдёт, если Debezium нуждается в транзакциях из диапазона gtid_purged?"
  answer="Если нужные GTID находятся в purged диапазоне, значит binlog файлы с этими транзакциями уже удалены. Debezium не сможет продолжить чтение, и потребуется полный resnapshot базы данных, что может занять часы для больших таблиц."
/>
