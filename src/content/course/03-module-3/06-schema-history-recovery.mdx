...
### Scenario A: Normal Connector Restart <Icon name="check" className="text-emerald-400 inline" size="1.2em" />
...
### Scenario B: Schema History Topic Partially Purged <Icon name="warning" className="text-yellow-400 inline" size="1.2em" />
...
### Scenario C: Schema History Topic Corrupted <Icon name="alert" className="text-red-400 inline" size="1.2em" />
...
### Scenario D: Multiple Connectors Sharing Topic <Icon name="alert" className="text-red-400 inline" size="1.2em" />
...
// Connector A
{
  ...
  "config": {
    "schema.history.internal.kafka.topic": "schema-changes.shared"  // [NO] WRONG
  }
}
...
// Connector A
{
  "name": "mysql-connector-a",
  "config": {
    "schema.history.internal.kafka.topic": "schema-changes.mysql-connector-a"  // [OK] UNIQUE
  }
}
...
// Connector B
{
  "name": "mysql-connector-b",
  "config": {
    "schema.history.internal.kafka.topic": "schema-changes.mysql-connector-b"  // [OK] UNIQUE
  }
}
```
...
```sql
-- [OK] ADD COLUMN (nullable или с default)
ALTER TABLE customers ADD COLUMN phone VARCHAR(20);
ALTER TABLE customers ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP;

-- [OK] ADD INDEX (не влияет на CDC)
CREATE INDEX idx_email ON customers(email);

-- [OK] RENAME COLUMN (Debezium видит новое имя)
