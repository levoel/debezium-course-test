...
import { BinlogVsWalDiagram, RowFormatDiagram, BinlogEventSequenceDiagram, BinlogRotationDiagram } from '../../../components/diagrams/module3/BinlogArchitectureDiagrams';
...
<BinlogVsWalDiagram client:visible />
...
<RowFormatDiagram client:visible />
...
<KnowledgeCheck
  question="Почему формат ROW является единственным подходящим для CDC, а STATEMENT — нет?"
  answer="ROW формат записывает фактические значения строк до и после изменения, что обеспечивает детерминированность. STATEMENT записывает SQL-запросы как есть, и недетерминированные функции (NOW(), RAND(), UUID()) дают разные результаты на разных серверах."
/>
...
| **CDC совместимость** | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Да | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Нет | <Icon name="warning" className="text-yellow-400 inline" size="1.2em" /> Не рекомендуется |
...
<BinlogEventSequenceDiagram client:visible />
...
<BinlogRotationDiagram client:visible />

<KnowledgeCheck
  question="Из каких двух частей состоит позиция записи в MySQL binlog, и для чего Debezium её использует?"
  answer="Позиция состоит из имени файла (например, mysql-bin.000003) и смещения в байтах (offset). Debezium отслеживает эту позицию, чтобы при перезапуске продолжить чтение изменений с того места, где остановился."
/>
...
| **Плагин декодирования** | pgoutput (wal2json — discontinued) | Не требуется |
