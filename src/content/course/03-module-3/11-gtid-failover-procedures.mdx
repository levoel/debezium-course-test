...
sequenceDiagram
    ...
    participant P2 as Replica → Primary

    D->>P1: Читаю binlog: mysql-bin.000015:2548
    Note over P1: PRIMARY CRASHES
    P1--xD: Connection lost
    Note over P2: Promoted to PRIMARY
    D->>P2: Пытаюсь читать mysql-bin.000015:2548
    P2-->>D: ERROR: File not found or wrong position
    Note over D: MANUAL INTERVENTION REQUIRED
    Note over D: Инженер должен найти соответствующую позицию<br/>на новом primary вручную
    Note over D: Или сделать FULL RESNAPSHOT (часы/дни)
```
</Mermaid>
...
sequenceDiagram
    ...
    D->>P1: Читаю binlog: GTID=...562:1-1000
    Note over P1: PRIMARY CRASHES
    P1--xD: Connection lost
    Note over P2: Promoted to PRIMARY
    D->>P2: Подключаюсь с GTID offset: ...562:1000
    D->>P2: Запрашиваю события после GTID ...562:1000
    P2->>D: Автоматически нахожу позицию
    P2->>D: Отдаю события начиная с ...562:1001
    Note over D,P2: CDC продолжается БЕЗ MANUAL INTERVENTION
```
</Mermaid>
...
```mermaid
timeline
    title Heartbeat Protection During Idle Period
    Day 1 : Last INSERT в таблицу customers
          : Offset: GTID ...562:1000
    Day 2-6 : Таблица IDLE (no writes)
            : Heartbeat updates offset каждые 10s
            : Offset: GTID ...562:1050 (heartbeat GTIDs)
    Day 7 : Binlog retention 7 дней
          : mysql-bin.000001-000010 purged
    Day 8 : Primary fails, failover to replica
          : Offset указывает на GTID ...562:1050
          : NEW PRIMARY имеет эту GTID (не purged)
          : Connector продолжает работу!
```
</Mermaid>
...
```mermaid
graph TD
    A[Start Pre-Failover Validation] --> B{Connector RUNNING?}
    B -->|No| B1[Fix connector - abort failover]
    B -->|Yes| C{Lag < 5 seconds?}
    C -->|No| C1[Wait for lag to decrease]
    C -->|Yes| D{GTID mode ON?}
    D -->|No| D1[Enable GTID - abort failover]
    D -->|Yes| E{Record Executed_Gtid_Set}
    E --> F{Heartbeat events flowing?}
    F -->|No| F1[Fix heartbeat - abort failover]
    F -->|Yes| G[All checks passed - SAFE TO FAILOVER]
...
sequenceDiagram
    ...
    participant P1 as Primary (instance-1)
    participant R2 as Replica (instance-2)
    participant RDS as RDS Control Plane

    App->>EP: Write request
    EP->>P1: Route to primary
    Note over P1: PRIMARY CRASHES
    P1--xEP: Connection lost
    EP-->>App: Connection error
    Note over RDS: Detects failure (30-60s)
    RDS->>R2: Promote to PRIMARY
    R2->>R2: SET read_only=OFF
    RDS->>EP: Update DNS (10-30s)
    App->>EP: Retry write request
    EP->>R2: Route to NEW primary
    R2-->>App: Success
```
</Mermaid>
...
```mermaid
sequenceDiagram
    participant DBA as Engineer
    participant NP as New Primary
    participant D as Debezium
    participant K as Kafka Offset Storage

    DBA->>NP: SHOW GLOBAL VARIABLES LIKE 'gtid_executed'
    NP-->>DBA: ...562:1-10550
    DBA->>K: Check Debezium offset
    K-->>DBA: gtid_set: ...562:1-10500
    Note over DBA: GTID continuity verified:<br/>Offset (10500) < Executed (10550)<br/>Gap = 50 GTIDs = normal lag
```
</Mermaid>
...
graph LR
    subgraph "US-East-1 (Primary Region)"
        P1[Primary Writer<br/>UUID: 3e11fa47...]
        R1[Reader Instance]
    end

    subgraph "EU-West-1 (Secondary Region)"
        P2[Primary Writer<br/>UUID: 4f22gb58...]
        R2[Reader Instance]
    end

    P1 -->|Replication<br/>< 1 second| P2
