...
<KnowledgeCheck
  question="Почему heartbeat.interval.ms обязателен для production GTID failover и что происходит без него?"
  answer="Без heartbeat offset connector обновляется только при наличии реальных изменений в таблицах. Если таблицы idle, offset остаётся на старом GTID. Со временем binlog файлы с этим GTID purge удаляются. При failover connector не может найти нужную позицию, и требуется полный resnapshot, что означает часы или дни downtime для больших таблиц."
/>
...
if [ "$CONNECTOR_STATE" != "RUNNING" ]; then
  echo "  [NO] Connector not RUNNING (state: $CONNECTOR_STATE)"
...
echo "  [OK] Current GTID: $CURRENT_GTID"
...
if [ "$MAX_LAG" -gt 1000 ]; then
  echo "  [WARN] High lag: $MAX_LAG events"
  echo "  Recommendation: Wait for lag < 1000 before failover"
  read -p "  Continue anyway? (yes/no): " CONFIRM
  if [ "$CONFIRM" != "yes" ]; then exit 1; fi
else
  echo "  [OK] Lag acceptable: $MAX_LAG events"
fi
...
echo "  [OK] New primary: $NEW_PRIMARY"
...
while [ $RETRY -lt 12 ]; do  # 2 minutes max
  ...
  if [ "$TASK_STATE" == "RUNNING" ]; then
    echo "  [OK] Connector task recovered"
    break
  else
...
if [ $RETRY -eq 12 ]; then
  echo "  [NO] Connector task not recovered after 2 minutes"
...
echo "  [OK] GTID continuity (manual verification required)"
...
docker compose exec kafka kafka-console-consumer \
  ...
  --from-beginning \
  --timeout-ms 5000 \
  | grep -q "$TEST_EMAIL" && echo "  [OK] CDC event confirmed" || echo "  [NO] CDC event not found"
