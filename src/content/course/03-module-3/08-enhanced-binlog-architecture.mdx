...
import { EnhancedBinlogArchitectureDiagram, EnhancedVsStandardDiagram, RetentionComparisonDiagram } from '../../../components/diagrams/module3/EnhancedBinlogDiagrams';
...
<EnhancedVsStandardDiagram client:visible />
...
<EnhancedBinlogArchitectureDiagram client:visible />
...
<KnowledgeCheck
  question="Как Enhanced Binlog снижает compute overhead с 50% до 13%?"
  answer="В стандартном binlog MySQL движок последовательно записывает данные и сам сортирует события. Enhanced Binlog отправляет несортированные события параллельно с transaction log на специализированные storage nodes, которые выполняют сортировку. Движок освобождается от этой работы, что снижает нагрузку."
/>
...
<RetentionComparisonDiagram client:visible />
...
# Output:
# 0  [OK] Можно включать Enhanced Binlog
# > 0  [NO] НЕЛЬЗЯ включать Enhanced Binlog
```
...
<KnowledgeCheck
  question="Почему Enhanced Binlog несовместим с Aurora backtrack, и как это проверить?"
  answer="Enhanced Binlog хранит события в специализированных storage nodes, а backtrack изменяет состояние transaction log без синхронизации с этим хранилищем. Если кластер когда-либо имел backtrack (BacktrackWindow > 0), Enhanced Binlog нельзя включить. Проверка: aws rds describe-db-clusters --query BacktrackWindow."
/>
...
|----------|------------------|-----------|
| **Высокая write throughput, production CDC** | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> **Да** | 13% compute overhead vs 50%; 40% прирост throughput |
| **Нужен backtrack для point-in-time testing** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | Несовместимо — backtrack блокирует Enhanced Binlog |
| **Aurora Global Database с cross-region failover** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | Binlog не реплицируется в secondary регион, после failover требуется resnapshot |
| **Нужен binlog в backups для disaster recovery** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | Enhanced Binlog не включается в Aurora snapshots, после restore binlog теряется |
| **Стандартная workload, простота важнее оптимизации** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | Standard binlog проще, без ограничений backtrack/Global Database |
| **Single-region Aurora, high concurrency, no backtrack** | <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> **Да** | Ideal scenario для Enhanced Binlog |
| **Development/testing окружение с frequent clones** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | Clone не содержит Enhanced Binlog, каждый clone требует resnapshot |
| **CDC + Aurora automated backups для DR** | <Icon name="alert" className="text-red-400 inline" size="1.2em" /> **Нет** | После restore binlog позиция потеряна |

### Decision Flowchart

{/* Decision flowchart is covered by the critical limitations section in RetentionComparisonDiagram above */}
...
4. **Decision criteria:**
   - <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Используйте: high write throughput, single-region, no backtrack, можете принять resnapshot после restore
   - <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Не используйте: backtrack нужен, Global Database с cross-region CDC, binlog в backups критичен для DR
