...
import { BinlogRetentionFlowDiagram, HeartbeatMonitoringDiagram, RetentionConfigDiagram } from '../../../components/diagrams/module3/BinlogRetentionDiagrams';
...
<BinlogRetentionFlowDiagram client:visible />
...
<KnowledgeCheck
  question="По какой формуле рассчитывается оптимальное значение binlog_expire_logs_seconds для CDC?"
  answer="Формула: Retention = Max Expected Debezium Downtime + Safety Margin. Например, если максимальный простой Debezium составляет 2 дня, с запасом 2x retention должен быть 4 дня (345600 секунд). Это баланс между использованием диска и защитой от потери позиции."
/>
...
<RetentionConfigDiagram client:visible />
...
<HeartbeatMonitoringDiagram client:visible />
...
<KnowledgeCheck
  question="Как heartbeat механизм решает проблему idle-таблиц в CDC?"
  answer="Heartbeat периодически записывает данные в специальную таблицу, создавая события в binlog. Debezium читает эти события и обновляет свой offset, который всегда указывает на свежий binlog файл. Благодаря этому старые файлы могут быть безопасно удалены."
/>
