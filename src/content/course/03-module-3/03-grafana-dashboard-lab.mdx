...
graph TB
    subgraph "Row 1: Health Overview"
        A[Status<br/>Stat Panel]
        B[Current Lag<br/>Gauge]
        C[Events/sec<br/>Stat]
        D[Queue Usage<br/>Gauge]
    end

    subgraph "Row 2: Time Series"
        E[Lag Trend<br/>Time Series]
        F[Throughput<br/>Time Series]
        G[Queue Utilization<br/>Time Series]
    end

    subgraph "Row 3: Details"
        H[Staleness<br/>Time Series]
        I[Transactions<br/>Time Series]
    end
...
<Mermaid chart={`
graph LR
    subgraph "Row 1: Health Overview (4 panels)"
        A["Connected"]
        B["0.3s lag"]
        C["125.4 events/sec"]
        D["12% queue"]
...
<Mermaid chart={`
graph TB
    subgraph "Здоровый CDC Pipeline"
        A["Status: Connected"]
        B["Lag: 0.1-0.5s"]
        C["Queue: 5-20%"]
        D["Throughput: стабильный"]
    end

    subgraph "Требует внимания"
        E["Status: Connected"]
        F["Lag: 5-30s Warning"]
        G["Queue: 50-80%"]
        H["Throughput: снижается"]
    end

    subgraph "Критическое состояние"
        I["Status: Connected или нет"]
        J["Lag: более 30s Critical"]
        K["Queue: более 90%"]
        L["Throughput: падает/нуль"]
    end
