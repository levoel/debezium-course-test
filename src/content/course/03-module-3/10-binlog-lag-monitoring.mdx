...
import { LagMetricsFlowDiagram, MonitoringArchitectureDiagram } from '../../../components/diagrams/module3/MonitoringDiagrams';
...
<MonitoringArchitectureDiagram client:visible />
...
<KnowledgeCheck
  question="Как рассчитывается JMX-метрика MilliSecondsBehindSource и почему она предпочтительнее SecondsBehindMaster?"
  answer="MilliSecondsBehindSource рассчитывается как разница между текущим системным временем и timestamp из binlog event (source.ts_ms). Она предпочтительнее, потому что обеспечивает точность в миллисекундах (а не секундах), рассчитывается самим Debezium на основе реальных event timestamps, и доступна во всех конфигурациях, тогда как SecondsBehindMaster может быть unavailable."
/>
...
<LagMetricsFlowDiagram client:visible />
...
<KnowledgeCheck
  question="Что означает QueueRemainingCapacity в метриках Debezium и о чём сигнализирует заполнение очереди более чем на 80%?"
  answer="QueueRemainingCapacity показывает оставшееся место во внутренней очереди Debezium между binlog reader thread и Kafka producer thread. Заполнение более 80% означает backpressure — Debezium читает binlog быстрее, чем может записывать в Kafka. Это приводит к росту latency, давлению на JVM heap и риску потери событий. Причинами обычно являются проблемы производительности Kafka broker."
/>
