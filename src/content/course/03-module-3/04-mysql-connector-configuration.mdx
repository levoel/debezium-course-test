...
sequenceDiagram
    ...
    Note over C: Snapshot Phase START
    C->>M: SET GLOBAL read_lock = ON (if allowed)
    C->>M: SELECT * FROM customers
    M-->>C: 100,000 rows
    C->>K: Publish 100,000 events to mysql-server.inventory.customers
    C->>M: SELECT * FROM orders
    M-->>C: 500,000 rows
    C->>K: Publish 500,000 events to mysql-server.inventory.orders
    C->>M: UNLOCK TABLES
    Note over C: Snapshot Phase COMPLETE

    Note over C: Streaming Phase START
    C->>M: Start reading binlog from GTID position
    M-->>C: Stream INSERT/UPDATE/DELETE events
    C->>K: Publish real-time CDC events
```
</Mermaid>
