...
<table style={{fontSize: '0.9em'}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Нет Binlog streaming blocked до завершения snapshot</td>
      <td>Да Binlog streaming продолжается параллельно</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Нет Restart from scratch</td>
      <td>Да Resume from last completed chunk</td>
    </tr>
    <tr>
...
|--------|------------|----------|----------------------------------------------|
| `id`   | VARCHAR(36)| Да       | Unique ID (рекомендуется UUID())             |
| `type` | VARCHAR(32)| Да       | Signal type: execute-snapshot, stop-snapshot |
| `data` | TEXT       | Да       | JSON payload (может быть NULL для некоторых signals) |
...
Если `inventory` в `database.include.list` — <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> работает.
...
```sql
-- [NO] НЕ работает на read-only replica
...
**Модуль 8 roadmap:**
- <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Lesson 1-9: MySQL binlog, GTID, Aurora parameter groups, snapshot modes
- <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> **Lesson 12: Incremental snapshots** ← Вы здесь
- <Icon name="refresh" className="text-blue-400 inline" size="1.2em" /> Lesson 13: Production troubleshooting
- <Icon name="refresh" className="text-blue-400 inline" size="1.2em" /> Lesson 14: MySQL/Aurora decision matrix
