...
<KnowledgeCheck
  question="Если в конфигурации коннектора указан topic.prefix=myapp и отслеживается таблица public.users, какое имя получит Kafka-топик?"
  answer="Топик получит имя myapp.public.users. Debezium формирует имя по шаблону {topic.prefix}.{schema}.{table}. Эта трехкомпонентная схема предотвращает коллизии имен, если одинаковые таблицы существуют в разных схемах базы данных."
/>
...
<FirstConnectorDiagram client:load />
