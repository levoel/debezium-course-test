...
<LabSetupDiagram client:load />

<KnowledgeCheck
  question="Почему лабораторное окружение использует порт 5433 для PostgreSQL вместо стандартного 5432?"
  answer="Порт 5432 часто занят локальной установкой PostgreSQL на компьютере разработчика. Docker маппит внешний порт 5433 на внутренний 5432 контейнера, позволяя обоим PostgreSQL-серверам работать одновременно. Внутри Docker-сети коннектор Debezium обращается к PostgreSQL по стандартному порту 5432."
/>
