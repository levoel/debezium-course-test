...
<Mermaid chart={`
flowchart TB
    subgraph "Типы операций CDC"
        R["r (snapshot)<br/>before: null<br/>after: данные"]
        C["c (create)<br/>before: null<br/>after: данные"]
        U["u (update)<br/>before: старое<br/>after: новое"]
        D["d (delete)<br/>before: данные<br/>after: null"]
    end
...
graph TB
    ...
    Event --> Schema[schema<br/>JSON Schema структуры]
    Event --> Payload[payload<br/>Основные данные]

    Payload --> Before[before<br/>Состояние ДО]
    Payload --> After[after<br/>Состояние ПОСЛЕ]
    Payload --> Source[source<br/>Метаданные]
    Payload --> Op[op<br/>Тип операции]
    Payload --> TsMs[ts_ms<br/>Время обработки]

    Source --> Db[db, schema, table]
    Source --> Lsn[lsn, txId]
