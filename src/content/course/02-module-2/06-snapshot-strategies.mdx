...
<Mermaid chart={`
graph LR
    subgraph "Без snapshot"
        A[Существующие данные] -->|Невидимы| X((Потеряны))
        B[Новые изменения] -->|Захватываются| K[Kafka]
    end
...
sequenceDiagram
    ...
    rect rgb(255, 200, 200)
        Note over C,K: Период блокировки streaming
        C->>PG: SELECT * FROM large_table (SNAPSHOT)
        Note over PG: 10 миллионов строк...
        Note over PG: WAL накапливается
        PG-->>C: Все строки
        C->>K: op='r' events
    end

    rect rgb(200, 255, 200)
        Note over C,K: Streaming возобновляется
        C->>PG: Чтение WAL
        PG-->>C: Накопленные изменения
        C->>K: op='c/u/d' events
    end
`} client:visible />
...
sequenceDiagram
    ...
    participant PG as PostgreSQL
    participant K as Kafka
    participant S as Signaling Table

    Note over C,K: Параллельная работа

    rect rgb(200, 255, 200)
        S->>C: execute-snapshot signal
        loop Chunk by chunk
            C->>PG: SELECT * WHERE id BETWEEN x AND y
            PG-->>C: Chunk (2048 строк)
            C->>K: op='r' (snapshot events)
        end
    end

    par Одновременно
        PG->>C: WAL changes
        C->>K: op='c/u/d' (streaming events)
    end

    Note over K: Оба потока событий в топике
`} client:visible />
...
<Mermaid chart={`
flowchart TD
    A[Нужен снэпшот?] -->|Да| B{Размер таблицы}
    A -->|Нет| Z1["snapshot.mode=never"]

    B -->|< 1M строк| C{Допустимый downtime?}
    B -->|>= 1M строк| D[Incremental Snapshot]

    C -->|Да, до 30 мин| E["snapshot.mode=initial"]
    C -->|Нет| D

    D --> F{Нужна фильтрация?}
    F -->|Нет| G["Полный incremental"]
    F -->|Да| H["additional-conditions"]

    style D fill:#10b981,color:#fff
    style Z1 fill:#6366f1,color:#fff
