...
<TraditionalSnapshotDiagram client:visible />
...
<IncrementalSnapshotDiagram client:visible />
...
<SnapshotDecisionDiagram client:visible />
...
<KnowledgeCheck
  question="Почему snapshot.mode=always — анти-паттерн в production?"
  answer="Каждый рестарт коннектора вызывает полный snapshot, повторно отправляя все данные в Kafka. Это создаёт дублирование событий, блокирует streaming на время snapshot и вызывает рост WAL retention. В production используйте initial (первый запуск) или never с incremental snapshot."
/>
