...
| **Плагин вывода** | Любой (pgoutput; wal2json — discontinued) | pgoutput (wal2json предустановлен, но discontinued — используйте pgoutput) |
...
<AuroraParameterGroupDiagram client:load />
...
<KnowledgeCheck
  question="Инженер ищет параметр rds.logical_replication в DB Instance Parameter Group, но не находит его. В чём ошибка?"
  answer="rds.logical_replication доступен только в DB Cluster Parameter Group, а не в DB Instance Parameter Group. Это критическое различие в Aurora: параметры уровня кластера (WAL, репликация) управляются через кластерную группу параметров."
/>
...
<KnowledgeCheck
  question="После применения parameter group к кластеру, SHOW wal_level; возвращает 'replica'. Какой шаг был пропущен?"
  answer="Writer Instance не был перезагружен. В Aurora изменения parameter group требуют reboot для применения. Без него старые параметры остаются активными. Это самая частая ошибка при настройке CDC на Aurora."
/>
