...
<AuroraFailoverSequenceDiagram client:load />
...
<KnowledgeCheck
  question="Heartbeat предотвращает потерю данных при Aurora failover?"
  answer="Нет. Heartbeat только обнаруживает пропуски в CDC-потоке, но не может их предотвратить. Его ценность — в раннем оповещении: когда разрыв heartbeat превышает настроенный интервал, это сигнал о произошедшем инциденте (failover), и можно запустить процедуры восстановления, например incremental snapshot."
/>
...
<AuroraGlobalDatabaseDiagram client:visible />
