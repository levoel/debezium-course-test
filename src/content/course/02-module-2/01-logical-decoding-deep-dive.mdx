...
import { PhysicalVsLogicalDiagram, LogicalDecodingComponentsDiagram, PublicationsDiagram, LogicalDecodingSequenceDiagram, WalRecordDiagram } from '../../../components/diagrams/module2';
...
<LogicalDecodingComponentsDiagram client:visible />
...
| wal2json | Сторонний, **discontinued** | JSON формат; поддержка прекращена сообществом — для нового кода используйте встроенный pgoutput |
...
<KnowledgeCheck
  question="Почему pgoutput рекомендуется для Debezium, даже если wal2json выдаёт более читаемый JSON?"
  answer="pgoutput встроен в PostgreSQL 10+ и не требует установки дополнительного ПО. Он работает на всех managed PostgreSQL (Aurora, RDS, Cloud SQL). wal2json — сторонний плагин, который часто невозможно установить на managed-сервисах; кроме того, его поддержка прекращена (discontinued), поэтому для нового кода всегда используйте встроенный pgoutput."
/>
...
<WalRecordDiagram client:visible />
...
<PublicationsDiagram client:visible />
...
<KnowledgeCheck
  question="Таблица имеет REPLICA IDENTITY DEFAULT. Какие данные будут в поле before при UPDATE-событии?"
  answer="Только колонки Primary Key. При DEFAULT PostgreSQL записывает в WAL минимум информации для идентификации строки. Для получения всех колонок в before необходимо переключить на REPLICA IDENTITY FULL, но это увеличит объём WAL на 30-50% для UPDATE-heavy таблиц."
/>
...
<LogicalDecodingSequenceDiagram client:visible />
