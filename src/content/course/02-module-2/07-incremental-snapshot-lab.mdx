...
graph LR
    subgraph PostgreSQL
        ST[debezium_signal]
        OL[orders_large<br/>10,000 rows]
    end
    ...
    subgraph Kafka
        T[inventory.public.orders_large]
    end
    ...
    style OL fill:#4169E1,color:#fff
    style C fill:#ef4444,color:#fff
    style T fill:#000,color:#fff
...
<Mermaid chart={`
graph TD
    A[Signaling Table создана] --> B[Orders Large: 10K записей]
    B --> C[Коннектор с incremental snapshot]
    C --> D[Python Monitor запущен]
    D --> E[INSERT execute-snapshot]
    E --> F[Наблюдение chunk-by-chunk]
    F --> G[Stop/Resume демонстрация]
    G --> H[Filtered snapshot]
