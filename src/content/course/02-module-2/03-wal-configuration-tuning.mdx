...
<Mermaid chart={`
flowchart TB
    subgraph LEVELS["Иерархия wal_level"]
        direction TB
        MINIMAL["minimal<br/>Только crash recovery<br/>Минимальный объем WAL"]
        REPLICA["replica<br/>+ Physical replication<br/>+ Point-in-time recovery"]
        LOGICAL["logical<br/>+ Logical decoding<br/>+ CDC (Debezium)"]

        MINIMAL --> REPLICA --> LOGICAL
    end

    MINIMAL -.->|"Недостаточно<br/>для репликации"| X1[" "]
    REPLICA -.->|"Недостаточно<br/>для CDC"| X2[" "]
    LOGICAL -.->|"Требуется<br/>для Debezium"| CHECK["Debezium Ready"]

    style MINIMAL fill:#7f1d1d,stroke:#ef4444
    style REPLICA fill:#1e3a5f,stroke:#3b82f6
...
<Mermaid chart={`
flowchart LR
    subgraph WORKLOAD["Тип нагрузки"]
        INSERT["INSERT-heavy<br/>Много вставок"]
        MIXED["Mixed workload<br/>INSERT + UPDATE"]
        UPDATE["UPDATE-heavy<br/>Много обновлений"]
    end

    subgraph IMPACT["Увеличение WAL"]
        I1["+ 5%"]
        I2["+ 10%"]
        I3["+ 15-30%"]
    end
    ...
    style INSERT fill:#14532d,stroke:#22c55e
    style MIXED fill:#1e3a5f,stroke:#3b82f6
    style UPDATE fill:#7f1d1d,stroke:#ef4444
`} client:visible />
