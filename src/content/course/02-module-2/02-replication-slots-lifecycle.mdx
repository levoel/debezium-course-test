...
<WalRetentionDiagram client:load />
...
<SlotLifecycleDiagram client:visible />
...
<KnowledgeCheck
  question="Слот показывает wal_status='extended'. Это нормально или повод для беспокойства?"
  answer="extended означает, что WAL превысил max_wal_size, но слот удерживает сегменты. Это предупреждающий сигнал — ещё не критическое состояние (критическое — unreserved или lost), но необходимо выяснить причину отставания потребителя."
/>
...
<KnowledgeCheck
  question="Что происходит, когда WAL retention слота превышает max_slot_wal_keep_size?"
  answer="PostgreSQL получает право удалить WAL-сегменты, даже если слот их не прочитал. Статус слота меняется на unreserved, затем на lost. Коннектору потребуется полный resnapshot для восстановления. Это намеренное поведение — защита дискового пространства приоритетнее непрерывности CDC."
/>
