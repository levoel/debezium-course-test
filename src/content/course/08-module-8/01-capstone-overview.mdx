...
<KnowledgeCheck
  question={'Какие пять категорий deliverables должен содержать capstone проект для признания его production-ready?'}
  answer={'(1) Working Infrastructure — Docker Compose/Kubernetes с полным стеком компонентов. (2) Code & Configuration — DDL, connector config, PyFlink job. (3) Monitoring & Observability — Prometheus + Grafana с ключевыми метриками. (4) Documentation — README, architecture.md, runbook.md. (5) Testing Evidence — скриншоты или логи end-to-end прохождения события через pipeline.'}
/>
