...
import { SystemContextDiagram, ContainerDiagram, CapstoneProjectStructureDiagram } from '../../../components/diagrams/module8';

# Архитектура и Deliverables

В [предыдущем уроке](/course/08-module-8/01-capstone-overview) вы узнали о **цели capstone проекта** — построить production-ready CDC pipeline. Теперь детализируем **архитектуру** и **точные deliverables**, которые вам нужно создать.
...
<CapstoneProjectStructureDiagram client:visible />
...
<ContainerDiagram client:visible />

**Цель:** Показать технологии каждого компонента и data flow между ними.

<KnowledgeCheck
  question={'В C4 модели какой уровень диаграммы показывает внешних пользователей и системы, а какой — внутренние компоненты pipeline?'}
  answer={'System Context (Level 1) показывает внешних пользователей и системы, с которыми взаимодействует CDC pipeline — бизнес-аналитиков, e-commerce приложение, BigQuery. Container Diagram (Level 2) показывает внутренние компоненты: Aurora PostgreSQL, Kafka, Debezium Connect, PyFlink, BigQuery Sink и их взаимодействия.'}
/>
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 1: Missing REPLICA IDENTITY FULL
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 2: No Primary Key in BigQuery
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 3: snapshot.mode=always
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 4: Нет мониторинга replication slot
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 5: Hardcoded credentials
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Pattern 6: Нет checkpointing в PyFlink
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Pattern 1: Heartbeat для replication slot
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Pattern 2: At-least-once delivery с idempotency

**Зачем:** По умолчанию CDC дает at-least-once; downstream должен быть idempotent.
...
> **Debezium 3.3+ exactly-once semantics.** С версии 3.3 (октябрь 2025) Debezium поддерживает exactly-once source semantics для всех core connectors (PostgreSQL, MySQL, MariaDB, MongoDB, Oracle, SQL Server). Включается на уровне Kafka Connect 3.3+ через worker-настройку `exactly.once.source.support=enabled` (плюс distributed mode и transactional producer). Это убирает дубликаты на стороне Kafka, но не отменяет требование идемпотентности downstream при доставке в системы за пределами Kafka (BigQuery, S3, Pub/Sub) — там EOS зависит от sink-коннектора.

### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Pattern 3: Monitoring Four Golden Signals
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Pattern 4: Runbook для common failures
...
10. **Production patterns:** Heartbeat, at-least-once + idempotency (или exactly-once через `exactly.once.source.support=enabled` в Debezium 3.3+/Kafka Connect 3.3+), monitoring, runbook
