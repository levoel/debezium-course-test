...
В [предыдущем уроке](/course/08-module-8/04-multi-database-architecture) вы узнали об **архитектуре multi-database CDC pipeline** и паттернах объединения данных из PostgreSQL и MySQL. Теперь время перейти к **практической конфигурации** — настроить оба коннектора и создать unified PyFlink consumer.
...
MySQL binlog с ROW format автоматически захватывает полные данные строк — не требуется эквивалент PostgreSQL `REPLICA IDENTITY FULL`. Убедитесь, что binlog_format=ROW установлен (см. [Module 8 - MySQL Configuration](/course/03-module-3/01-mysql-aurora-setup)).
</Callout>
...
**Recommended range:** 184000-184999 для Debezium connectors (см. [Module 8 - Multi-Connector Deployment](/course/03-module-3/06-multi-connector-deployment) для registry pattern).
...
<MonitoringMultiDatabaseDiagram client:load />
...
<KnowledgeCheck
  question={'Зачем PyFlink multi-source consumer добавляет колонку source_database и composite_key при UNION ALL потоков из PostgreSQL и MySQL?'}
  answer={'source_database (postgresql/mysql) обеспечивает traceability: при troubleshooting можно быстро определить origin database для каждого события. composite_key (PG-{id}/MY-{id}) предотвращает конфликт primary key: order_id=5 в PostgreSQL и order_id=5 в MySQL — разные записи, но при MERGE в BigQuery по order_id одна перезапишет другую. Composite key гарантирует уникальность в unified stream.'}
/>
