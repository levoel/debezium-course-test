...
<ProductionGapDiagram client:load />
...
<FourGoldenSignalsDiagram client:visible />
...
<KnowledgeCheck
  question={'Назовите Four Golden Signals и укажите, какая метрика Debezium соответствует каждому сигналу для CDC pipeline.'}
  answer={'(1) Latency — MilliSecondsBehindSource (задержка репликации). (2) Traffic — events/second throughput (TotalNumberOfEventsSeen rate). (3) Errors — connector status RUNNING/FAILED и error rate. (4) Saturation — QueueRemainingCapacity (уровень заполнения внутренней очереди) и Kafka consumer lag.'}
/>
...
**Ошибка:** Downstream consumers используют INSERT вместо UPSERT — at-least-once delivery (default для Debezium) приводит к duplicates.
...
**Prevention:** Default to at-least-once + idempotency. Для пайплайнов, где идемпотентность невозможна, можно включить exactly-once source semantics в Debezium 3.3+ через worker-настройку Kafka Connect 3.3+ `exactly.once.source.support=enabled` (требуется distributed mode и transactional producer). EOS убирает дубликаты на стороне Kafka, но downstream sinks (BigQuery, S3) всё равно нуждаются в idempotent writes.

<KnowledgeCheck
  question={'Какой scoring level получит capstone проект, в котором pipeline работает end-to-end, connector restart протестирован, но Grafana dashboard отсутствует и нет runbook?'}
  answer={'Meets Expectations (60-74%). Core functionality works (Section 1, 2): pipeline передает данные, восстанавливается после connector restart. Но Monitoring incomplete (нет Grafana dashboard) и Documentation minimal (нет runbook). Для Above Average нужны: dashboard с Four Golden Signals и runbook с хотя бы 2-3 failure scenarios.'}
/>
...
**Полезные ресурсы:**
- [Multi-Database Architecture](/course/08-module-8/04-multi-database-architecture) — сравнение separate topics vs unified topics patterns
- [Multi-Database Configuration](/course/08-module-8/05-multi-database-configuration) — практическая имплементация
...
3. **Fault tolerance** критична: at-least-once delivery + idempotency = no data loss, no duplicates. Альтернатива в Debezium 3.3+ — exactly-once source semantics для всех core connectors (`exactly.once.source.support=enabled` в Kafka Connect 3.3+)
