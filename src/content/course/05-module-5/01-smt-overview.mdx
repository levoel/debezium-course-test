...
<div className="not-prose">
  <SmtSolutionDiagram client:visible />
</div>
...
<div className="not-prose">
  <SmtExecutionModelDiagram client:visible />
</div>
...
Debezium 3.4 предоставляет набор production-ready SMT для типичных сценариев.
...
<div className="not-prose">
  <SmtChainOrderDiagram client:visible />
</div>
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Good Use Cases
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Anti-Patterns
...
Debezium 3.4 **не предоставляет** метрики per-SMT. Но вы можете измерить общий impact:
...
<div className="not-prose">
  <SmtDecisionFrameworkDiagram client:visible />
</div>

## Полный каталог Debezium SMT

Для справки — все SMT, доступные в Debezium 3.4:
...
2. **Debezium 3.4** включает 6 production-ready SMT: Filter, Unwrap, Router (2 типа), Outbox, PartitionRouting
