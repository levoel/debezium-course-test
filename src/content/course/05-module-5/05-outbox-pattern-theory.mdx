...
<div className="not-prose">
  <OutboxSolutionDiagram client:visible />
</div>
...
**Гарантия:** Если транзакция зафиксирована — событие **гарантированно** попадет в Kafka. По умолчанию — at-least-once (consumers должны быть идемпотентны). Начиная с Debezium 3.3+ возможна exactly-once source semantics для всех core connectors через worker-настройку Kafka Connect 3.3+ `exactly.once.source.support=enabled` (требуется distributed mode и transactional producer); при включении дубликаты на стороне Kafka исключаются, но downstream sinks за пределами Kafka всё равно требуют idempotent writes.

<KnowledgeCheck
  question="Почему прямая отправка события в Kafka после COMMIT в базу данных (dual-write) не является надежным решением? Какую гарантию нарушает этот подход?"
  answer="Dual-write выполняет два независимых write-операции: COMMIT в БД и send() в Kafka. Между ними может произойти network failure, crash приложения или недоступность Kafka. В результате БД обновлена, но событие потеряно -- нарушается гарантия atomicity. Outbox Pattern решает это, сводя два write к одному: UPDATE + INSERT в outbox выполняются в одной транзакции БД, а CDC асинхронно доставляет событие."
/>
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Что Outbox ГАРАНТИРУЕТ
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Что Outbox НЕ ГАРАНТИРУЕТ
...
<div className="not-prose">
  <OutboxTransactionFlowDiagram client:visible />
</div>
...
<KnowledgeCheck
  question="Outbox Pattern гарантирует at-least-once delivery. Почему это означает, что consumers обязаны быть идемпотентными? Как реализуется deduplication?"
  answer="At-least-once означает, что событие доставится минимум один раз, но может дублироваться -- например, при restart Debezium может повторно прочитать WAL-сегмент. Consumer реализует deduplication по event ID (UUID из поля id outbox-таблицы): сохраняет обработанные ID в таблицу processed_events и проверяет перед обработкой. Если ID уже обработан -- событие пропускается."
/>
...
### <Icon name="check" className="text-emerald-400 inline" size="1.2em" /> Use Cases для Outbox
...
### <Icon name="alert" className="text-red-400 inline" size="1.2em" /> Когда Outbox избыточен
...
<div className="not-prose">
  <MicroservicesOutboxDiagram client:visible />
</div>
