...
<Mermaid chart={`
flowchart LR
    subgraph BATCH["Traditional Batch Pipeline"]
        DB["PostgreSQL"] -->|"Daily dump"| ETL["ETL Job<br/>Runs at 2am"]
        ETL --> DW["Data Warehouse"]
        DW -->|"Feature query"| FS["Feature Store"]
        FS -->|"Features<br/>(12h old)"| ML["ML Model"]
    end

    style ETL fill:#f59e0b
...
<Mermaid chart={`
flowchart LR
    subgraph RT["Real-time CDC Pipeline"]
        DB["PostgreSQL"] -->|"CDC events<br/>(ms latency)"| KAFKA["Kafka"]
        KAFKA --> SPARK["PySpark<br/>Feature Computation"]
        SPARK -->|"Fresh features<br/>(seconds old)"| REDIS["Feature Store<br/>(Redis)"]
        REDIS -->|"Real-time lookup"| ML["ML Model"]
    end

    style SPARK fill:#10b981
    style REDIS fill:#10b981
`} client:visible />
...
flowchart TB
    subgraph SOURCE["Source Database"]
        ORDERS_TABLE["orders table<br/>(PostgreSQL)"]
    end
    ...
    subgraph COMPUTE["Feature Computation"]
        PYSPARK["PySpark Streaming<br/>Window Aggregations"]
        WATERMARK["Watermark<br/>(1 hour late data)"]
    end

    subgraph SINK["Feature Store"]
        REDIS["Redis<br/>(Online Store)"]
        BATCH["Parquet<br/>(Offline Store)"]
    end

    ORDERS_TABLE -->|"CDC events"| DEBEZIUM
    DEBEZIUM -->|"Publish"| KAFKA
    KAFKA -->|"Read stream"| PYSPARK
    PYSPARK --> WATERMARK
    WATERMARK -->|"foreachBatch"| REDIS
    WATERMARK -->|"Batch export"| BATCH
...
flowchart TB
    subgraph OFFLINE["Offline Store (Training)"]
        ...
        FEAST_OFFLINE["Feast Offline API<br/>get_historical_features()"]
    end

    subgraph ONLINE["Online Store (Serving)"]
        REDIS_FEAST["Redis<br/>(Feature Cache)"]
        FEAST_ONLINE["Feast Online API<br/>get_online_features()"]
    end

    subgraph SOURCES["Feature Sources"]
        SPARK["PySpark Streaming"] --> REDIS_FEAST
        SPARK --> PARQUET
    end

    subgraph CONSUMERS["Consumers"]
        TRAINING["Training Pipeline"] --> FEAST_OFFLINE
        INFERENCE["Inference Service"] --> FEAST_ONLINE
    end

    FEAST_OFFLINE --> PARQUET
    FEAST_ONLINE --> REDIS_FEAST

    style FEAST_OFFLINE fill:#8b5cf6
    style FEAST_ONLINE fill:#10b981
`} client:visible />
...
|----------|-------------------|----------------|----------|
| Fraud detection | Real-time (seconds) | <1s | CDC → Streaming features |
| Product recommendations | Near real-time (minutes) | <5min | CDC → Micro-batch features |
