...
<Mermaid chart={`
flowchart LR
    subgraph INPUT["Input: CDC Event"]
        direction TB
        CDC_BEFORE["before: null"]
        CDC_AFTER["after:<br/>{<br/>  id: '550e8400...',<br/>  aggregatetype: 'Order',<br/>  aggregateid: 'order-123',<br/>  type: 'OrderApproved',<br/>  payload: {...}<br/>}"]
        CDC_OP["op: 'c' (create)"]
        CDC_SOURCE["source: {table: 'outbox', ...}"]
    end

    subgraph SMT["Outbox Event Router SMT"]
        ROUTE["Route by aggregatetype"]
        EXTRACT["Extract payload"]
        KEY["Set Kafka key = aggregateid"]
        HEADER["Add headers: id, type"]
    end

    subgraph OUTPUT["Output: Domain Event"]
        direction TB
        TOPIC["Topic: outbox.event.Order"]
        KEY_OUT["Key: 'order-123'"]
        VALUE_OUT["Value: {<br/>  orderId: '123',<br/>  amount: 299.99<br/>}"]
        HEADERS["Headers:<br/>  id: '550e8400...'<br/>  type: 'OrderApproved'"]
    end

    INPUT --> SMT
    SMT --> OUTPUT

    style INPUT fill:#ef4444
