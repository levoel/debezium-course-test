...
<KnowledgeCheck
  question="Параметр table.expand.json.payload в Outbox Event Router может быть true или false. Какая разница для consumer, и какой вариант рекомендуется?"
  answer="С table.expand.json.payload = false (default) consumer получает экранированную JSON строку, требующую дополнительного JSON.parse(). С true consumer получает structured JSON object напрямую. Рекомендуется true, потому что consumer code проще и type-safe -- не нужен дополнительный этап десериализации для разбора строки."
/>

## Consumer Idempotency Pattern

По умолчанию Outbox Pattern гарантирует **at-least-once delivery** → consumers должны обрабатывать duplicate events. (В Debezium 3.3+ для всех core connectors можно включить exactly-once source semantics через `exactly.once.source.support=enabled` в Kafka Connect 3.3+, тогда дубликаты на стороне Kafka исключаются — но идемпотентность downstream остаётся хорошей практикой.)
...
def handle_order_approved(event):
    ...
    print(f"[OK] Order {order_id} approved for ${amount}")
...
def handle_order_cancelled(event):
    ...
    order_id = event['orderId']
    print(f"[NO] Order {order_id} cancelled")
```
...
<Icon name="check" className="text-emerald-400 inline" size="1.2em" /> **Success!** Outbox Event Router преобразовал CDC event в domain event.
