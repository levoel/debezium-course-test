...
<KnowledgeCheck
  ...
  answer="С table.expand.json.payload = false (default) consumer получает JSON string в value: {\"orderId\": \"123\"} (экранированная строка). С true consumer получает structured JSON object: {orderId: 123}. Рекомендуется true, потому что consumer code проще и type-safe -- не нужен дополнительный этап JSON.parse() для разбора строки."
/>
