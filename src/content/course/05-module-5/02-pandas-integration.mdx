...
<Mermaid chart={`
graph TD
    Event[CDC Event JSON] --> Schema[schema]
    Event --> Payload[payload]

    Payload --> Before[before: {...}]
    Payload --> After[after: {...}]
    Payload --> Op[op: 'c' | 'u' | 'd' | 'r']
    Payload --> TsMs[ts_ms: 1738425600000]
    Payload --> Source[source: {...}]

    Before --> BeforeFields[id, customer_id, total, ...]
    After --> AfterFields[id, customer_id, total, ...]
    Source --> SourceFields[db, table, lsn, ...]

    style Payload fill:#4169E1,color:#fff
    style Op fill:#ff6b6b,color:#fff
    style Before fill:#51cf66,color:#000
    style After fill:#51cf66,color:#000
`} client:visible />
