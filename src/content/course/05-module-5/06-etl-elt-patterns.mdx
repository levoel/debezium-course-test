...
<Mermaid chart={`
flowchart LR
    SOURCE["Source DB<br/>(PostgreSQL)"] -->|Extract| STAGE["Staging Area"]
    STAGE -->|Transform| ETL["ETL Engine<br/>Filter, aggregate,<br/>join, clean"]
    ETL -->|Load| DW["Data Warehouse<br/>(Clean, aggregated)"]

    style ETL fill:#f59e0b
    style DW fill:#10b981
`} client:visible />
...
<Mermaid chart={`
flowchart LR
    SOURCE["Source DB<br/>(PostgreSQL)"] -->|Extract| CDC["CDC Stream<br/>(Debezium)"]
    CDC -->|Load Raw| LAKE["Data Lake<br/>(Parquet, Delta)"]
    LAKE -->|Transform| DW["Data Warehouse<br/>(dbt, Spark SQL)"]

    style LAKE fill:#3b82f6
    style DW fill:#10b981
`} client:visible />
...
<Mermaid chart={`
flowchart TB
    subgraph SOURCE["Transactional Database"]
        TX["Customer changes<br/>INSERT, UPDATE, DELETE"]
    end

    subgraph LAKE["Data Lake (Parquet)"]
        direction TB
        RAW["Raw CDC events<br/>(append-only log)"]
        SNAP["Latest snapshot<br/>(current state)"]
        HISTORY["Change history<br/>(audit trail)"]
    end
    ...
    style RAW fill:#3b82f6
    style SNAP fill:#10b981
    style HISTORY fill:#8b5cf6
`} client:visible />
...
<Mermaid chart={`
flowchart TB
    CDC["CDC Stream<br/>(Kafka)"] --> FILTER{"Operation Type"}

    FILTER -->|op = 'c'| INSERT["INSERT Stream"]
    FILTER -->|op = 'u'| UPDATE["UPDATE Stream"]
    FILTER -->|op = 'd'| DELETE["DELETE Stream"]

    INSERT -->|append| APPEND["Append Table<br/>(Parquet)"]
    UPDATE -->|upsert| UPSERT["Upsert Logic<br/>(Delta Lake MERGE)"]
    DELETE -->|soft delete| SOFT["Soft Delete<br/>(add deleted_at)"]

    style INSERT fill:#10b981
...
<Mermaid chart={`
flowchart LR
    KAFKA_IN["Kafka Source"] -->|exactly-once read| SPARK["PySpark Streaming"]
    SPARK -->|at-least-once write| KAFKA_OUT["Kafka Sink"]
    SPARK -->|idempotent write| PARQUET["Parquet/Delta"]

    style SPARK fill:#f59e0b
    style PARQUET fill:#10b981
    style KAFKA_OUT fill:#ef4444
`} client:visible />
