...
<div className="not-prose">
  <MultiTenantRoutingDiagram client:load />
</div>
...
<div className="not-prose">
  <ContentBasedRouterDiagram client:visible />
</div>
...
ContentBasedRouter требует **JSR 223 scripting engine** (Groovy). В Debezium 3.4 Groovy **не включен по умолчанию**.

**Добавление Groovy в Docker-образ:**

```dockerfile
# labs/Dockerfile.debezium-with-groovy
FROM quay.io/debezium/connect:3.4.0.Final
...
<div className="not-prose">
  <RegionBasedRoutingDiagram client:visible />
</div>
...
<KnowledgeCheck
  question="Чем отличается ByLogicalTableRouter от ContentBasedRouter? В каком сценарии нужен каждый?"
  answer="ByLogicalTableRouter маршрутизирует по имени топика (regex на имя таблицы) -- используется для объединения sharded таблиц в один топик. ContentBasedRouter маршрутизирует по значению поля в событии (Groovy expression) -- используется для multi-tenant routing, региональной маршрутизации, priority queues. Например, consolidation customers_shard_0/1/2 в один топик -- ByLogicalTableRouter, а routing по region EU/US -- ContentBasedRouter."
/>
...
**Бенчмарк (Debezium 3.4, 10K events/sec):**
