...
flowchart LR
    subgraph DB["PostgreSQL"]
        TABLE["customers table<br/>id, name, email, ssn"]
    end
    ...
    subgraph KAFKA["Kafka Topic"]
        MASKED["Masked Events<br/>email: ***MASKED***<br/>ssn: ***MASKED***"]
    end

    TABLE -->|"Full data"| CDC
    MASK -->|"Masked PII"| MASKED

    style MASK fill:#f59e0b
    style MASKED fill:#10b981
`} client:visible />
...
<Mermaid chart={`
sequenceDiagram
    participant E as Debezium Event
    participant U as ExtractNewRecordState
    participant M as MaskField
    participant K as Kafka

    E->>U: Full Debezium envelope
    Note over U: Extract "after" field<br/>Add metadata

    U->>M: Flattened record
    Note over M: Mask ssn, email fields

    M->>K: Masked flat record
`} client:visible />
